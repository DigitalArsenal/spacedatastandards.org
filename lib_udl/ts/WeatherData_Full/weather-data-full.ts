...
import { WeatherData_Full_dataMode_Enum } from './weather-data-full-data-mode-enum.js';
 ...
 */
export class WeatherData_Full implements flatbuffers.IUnpackableObject<WeatherData_FullT> {
  bb: flatbuffers.ByteBuffer|null = null;
  bb_pos = 0;
  __init(i:number, bb:flatbuffers.ByteBuffer):WeatherData_Full {
  this.bb_pos = i;
  this.bb = bb;
  return this;
}

static getRootAsWeatherData_Full(bb:flatbuffers.ByteBuffer, obj?:WeatherData_Full):WeatherData_Full {
  return (obj || new WeatherData_Full()).__init(bb.readInt32(bb.position()) + bb.position(), bb);
}

static getSizePrefixedRootAsWeatherData_Full(bb:flatbuffers.ByteBuffer, obj?:WeatherData_Full):WeatherData_Full {
 ...
 * Constraints: Minimum length = 1, Maximum length = 32
 */
dataMode():WeatherData_Full_dataMode_Enum {
  const offset = this.bb!.__offset(this.bb_pos, 80);
  return offset ? this.bb!.readInt8(this.bb_pos + offset) : WeatherData_Full_dataMode_Enum.REAL;
}

static startWeatherData_Full(builder:flatbuffers.Builder) {
...
unpack(): WeatherData_FullT {
  return new WeatherData_FullT(
    this.id(),
...
unpackTo(_o: WeatherData_FullT): void {
  _o.id = this.id();
...
export class WeatherData_FullT implements flatbuffers.IGeneratedObject {
constructor(
  ...
  public dataMode: WeatherData_Full_dataMode_Enum = WeatherData_Full_dataMode_Enum.REAL
){}
...
pack(builder:flatbuffers.Builder): flatbuffers.Offset {
  ...
  const sigPwrs = WeatherData_Full.createSigPwrsVector(builder, builder.createObjectOffsetList(this.sigPwrs));
  const noiseLvls = WeatherData_Full.createNoiseLvlsVector(builder, builder.createObjectOffsetList(this.noiseLvls));
  const specWidths = WeatherData_Full.createSpecWidthsVector(builder, builder.createObjectOffsetList(this.specWidths));
  const firstGuessAvgs = WeatherData_Full.createFirstGuessAvgsVector(builder, builder.createObjectOffsetList(this.firstGuessAvgs));
  const fileCreation = (this.fileCreation !== null ? builder.createString(this.fileCreation!) : 0);
  const tdAvgSampleNums = WeatherData_Full.createTdAvgSampleNumsVector(builder, builder.createObjectOffsetList(this.tdAvgSampleNums));
  const coIntegs = WeatherData_Full.createCoIntegsVector(builder, builder.createObjectOffsetList(this.coIntegs));
  const specAvgs = WeatherData_Full.createSpecAvgsVector(builder, builder.createObjectOffsetList(this.specAvgs));
  const interpulsePeriods = WeatherData_Full.createInterpulsePeriodsVector(builder, builder.createObjectOffsetList(this.interpulsePeriods));
  const doppVels = WeatherData_Full.createDoppVelsVector(builder, builder.createObjectOffsetList(this.doppVels));
  const consRecs = WeatherData_Full.createConsRecsVector(builder, builder.createObjectOffsetList(this.consRecs));
  const snrs = WeatherData_Full.createSnrsVector(builder, builder.createObjectOffsetList(this.snrs));
  const lightDetSensors = WeatherData_Full.createLightDetSensorsVector(builder, builder.createObjectOffsetList(this.lightDetSensors));
  const srcTyps = WeatherData_Full.createSrcTypsVector(builder, builder.createObjectOffsetList(this.srcTyps));
  const srcIds = WeatherData_Full.createSrcIdsVector(builder, builder.createObjectOffsetList(this.srcIds));
  ...
  return WeatherData_Full.createWeatherData_Full(builder,
    id,
