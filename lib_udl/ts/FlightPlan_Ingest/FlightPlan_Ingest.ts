...
import { FlightPlan_Ingest_dataMode_Enum } from './FlightPlan_Ingest_dataMode_Enum.js';


/**
 * /// Flight Plan contains data specifying the details of an intended flight including schedule and expected route.
 */
export class FlightPlan_Ingest implements flatbuffers.IUnpackableObject<FlightPlan_IngestT> {
  bb: flatbuffers.ByteBuffer|null = null;
  bb_pos = 0;
  __init(i:number, bb:flatbuffers.ByteBuffer):FlightPlan_Ingest {
  this.bb_pos = i;
  this.bb = bb;
  return this;
}

static getRootAsFlightPlan_Ingest(bb:flatbuffers.ByteBuffer, obj?:FlightPlan_Ingest):FlightPlan_Ingest {
  return (obj || new FlightPlan_Ingest()).__init(bb.readInt32(bb.position()) + bb.position(), bb);
}

static getSizePrefixedRootAsFlightPlan_Ingest(bb:flatbuffers.ByteBuffer, obj?:FlightPlan_Ingest):FlightPlan_Ingest {
...
dataMode():FlightPlan_Ingest_dataMode_Enum {
  ...
  return offset ? this.bb!.readInt8(this.bb_pos + offset) : FlightPlan_Ingest_dataMode_Enum.REAL;
}
...
static startFlightPlan_Ingest(builder:flatbuffers.Builder) {
...
static addDataMode(builder:flatbuffers.Builder, dataMode:FlightPlan_Ingest_dataMode_Enum) {
  builder.addFieldInt8(91, dataMode, FlightPlan_Ingest_dataMode_Enum.REAL);
}
...
unpack(): FlightPlan_IngestT {
  return new FlightPlan_IngestT(
    this.id(),
...
unpackTo(_o: FlightPlan_IngestT): void {
  _o.id = this.id();
...
export class FlightPlan_IngestT implements flatbuffers.IGeneratedObject {
constructor(
  ...
  public dataMode: FlightPlan_Ingest_dataMode_Enum = FlightPlan_Ingest_dataMode_Enum.REAL,
...
pack(builder:flatbuffers.Builder): flatbuffers.Offset {
  ...
  const countryCodes = FlightPlan_Ingest.createCountryCodesVector(builder, builder.createObjectOffsetList(this.countryCodes));
  const etopsRating = (this.etopsRating !== null ? builder.createString(this.etopsRating!) : 0);
  const etopsAirfields = FlightPlan_Ingest.createEtopsAirfieldsVector(builder, builder.createObjectOffsetList(this.etopsAirfields));
  const etopsAltAirfields = FlightPlan_Ingest.createEtopsAltAirfieldsVector(builder, builder.createObjectOffsetList(this.etopsAltAirfields));
  ...
  const atcAddresses = FlightPlan_Ingest.createAtcAddressesVector(builder, builder.createObjectOffsetList(this.atcAddresses));
  ...
  const airRefuelEvents = FlightPlan_Ingest.createAirRefuelEventsVector(builder, builder.createObjectOffsetList(this.airRefuelEvents));
  const flightPlanMessages = FlightPlan_Ingest.createFlightPlanMessagesVector(builder, builder.createObjectOffsetList(this.flightPlanMessages));
  const flightPlanWaypoints = FlightPlan_Ingest.createFlightPlanWaypointsVector(builder, builder.createObjectOffsetList(this.flightPlanWaypoints));
  const flightPlanPointGroups = FlightPlan_Ingest.createFlightPlanPointGroupsVector(builder, builder.createObjectOffsetList(this.flightPlanPointGroups));

  return FlightPlan_Ingest.createFlightPlan_Ingest(builder,
    id,
