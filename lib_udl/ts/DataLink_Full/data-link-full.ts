...
import { DataLink_Full_dataMode_Enum } from './data-link-full-data-mode-enum.js';


/**
 * /// Beta Version DataLink: Detailed instructions regarding the operations of data links.
 */
export class DataLink_Full implements flatbuffers.IUnpackableObject<DataLink_FullT> {
  bb: flatbuffers.ByteBuffer|null = null;
  bb_pos = 0;
  __init(i:number, bb:flatbuffers.ByteBuffer):DataLink_Full {
  this.bb_pos = i;
  this.bb = bb;
  return this;
}

static getRootAsDataLink_Full(bb:flatbuffers.ByteBuffer, obj?:DataLink_Full):DataLink_Full {
  return (obj || new DataLink_Full()).__init(bb.readInt32(bb.position()) + bb.position(), bb);
}

static getSizePrefixedRootAsDataLink_Full(bb:flatbuffers.ByteBuffer, obj?:DataLink_Full):DataLink_Full {
...
dataMode():DataLink_Full_dataMode_Enum {
  ...
  return offset ? this.bb!.readInt8(this.bb_pos + offset) : DataLink_Full_dataMode_Enum.REAL;
}

static startDataLink_Full(builder:flatbuffers.Builder) {
...
unpack(): DataLink_FullT {
  return new DataLink_FullT(
    this.id(),
...
unpackTo(_o: DataLink_FullT): void {
  _o.id = this.id();
...
export class DataLink_FullT implements flatbuffers.IGeneratedObject {
constructor(
  ...
  public dataMode: DataLink_Full_dataMode_Enum = DataLink_Full_dataMode_Enum.REAL
){}
...
pack(builder:flatbuffers.Builder): flatbuffers.Offset {
  ...
  const references = DataLink_Full.createReferencesVector(builder, builder.createObjectOffsetList(this.references));
  ...
  const canxSICs = DataLink_Full.createCanxSicsVector(builder, builder.createObjectOffsetList(this.canxSICs));
  ...
  const pocNums = DataLink_Full.createPocNumsVector(builder, builder.createObjectOffsetList(this.pocNums));
  const geoDatum = (this.geoDatum !== null ? builder.createString(this.geoDatum!) : 0);
  const refPoints = DataLink_Full.createRefPointsVector(builder, builder.createObjectOffsetList(this.refPoints));
  const voiceCoord = DataLink_Full.createVoiceCoordVector(builder, builder.createObjectOffsetList(this.voiceCoord));
  const sysDefaultCode = (this.sysDefaultCode !== null ? builder.createString(this.sysDefaultCode!) : 0);
  const multiDuty = DataLink_Full.createMultiDutyVector(builder, builder.createObjectOffsetList(this.multiDuty));
  const ops = DataLink_Full.createOpsVector(builder, builder.createObjectOffsetList(this.ops));
  ...
  const specTracks = DataLink_Full.createSpecTracksVector(builder, builder.createObjectOffsetList(this.specTracks));
  const nonLinkUnitDes = DataLink_Full.createNonLinkUnitDesVector(builder, builder.createObjectOffsetList(this.nonLinkUnitDes));
  const trackNumBlockLLs = DataLink_Full.createTrackNumBlockLlsVector(builder, builder.createObjectOffsetList(this.trackNumBlockLLs));
  const trackNumBlocks = DataLink_Full.createTrackNumBlocksVector(builder, builder.createObjectOffsetList(this.trackNumBlocks));
  ...
  const ackInstUnits = DataLink_Full.createAckInstUnitsVector(builder, builder.createObjectOffsetList(this.ackInstUnits));
  const classSource = (this.classSource !== null ? builder.createString(this.classSource!) : 0);
  const classReasons = DataLink_Full.createClassReasonsVector(builder, builder.createObjectOffsetList(this.classReasons));
  const decInstDates = DataLink_Full.createDecInstDatesVector(builder, builder.createObjectOffsetList(this.decInstDates));
  const decExemptCodes = DataLink_Full.createDecExemptCodesVector(builder, builder.createObjectOffsetList(this.decExemptCodes));
  const remarks = DataLink_Full.createRemarksVector(builder, builder.createObjectOffsetList(this.remarks));
  ...
  return DataLink_Full.createDataLink_Full(builder,
    id,
