...
class IonoObservationPhaseAbridged {
  IonoObservationPhaseAbridged._(this._bc, this._bcOffset);
  factory IonoObservationPhaseAbridged(List<int> bytes) {
    final rootRef = fb.BufferContext.fromBytes(bytes);
    return reader.read(rootRef, 0);
  }

  static const fb.Reader<IonoObservationPhaseAbridged> reader = _IonoObservationPhaseAbridgedReader();
  ...
  @override
  String toString() {
    return 'IonoObservationPhaseAbridged{notes: ${notes}, dimensions: ${dimensions}, dimensionName: ${dimensionName}, data: ${data}}';
  }
}

class _IonoObservationPhaseAbridgedReader extends fb.TableReader<IonoObservationPhaseAbridged> {
  const _IonoObservationPhaseAbridgedReader();

  @override
  IonoObservationPhaseAbridged createObject(fb.BufferContext bc, int offset) => 
    IonoObservationPhaseAbridged._(bc, offset);
}

class IonoObservationPhaseAbridgedBuilder {
  IonoObservationPhaseAbridgedBuilder(this.fbBuilder);
...
class IonoObservationPhaseAbridgedObjectBuilder extends fb.ObjectBuilder {
  final String? _notes;
  final List<String>? _dimensions;
  final List<String>? _dimensionName;
  final List<String>? _data;

  IonoObservationPhaseAbridgedObjectBuilder({
    String? notes,
