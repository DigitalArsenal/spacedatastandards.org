...
class TaiutcIngestDataModeEnum {
  final int value;
  const TaiutcIngestDataModeEnum._(this.value);

  factory TaiutcIngestDataModeEnum.fromValue(int value) {
    final result = values[value];
    if (result == null) {
        throw StateError('Invalid value $value for bit flag enum TaiutcIngestDataModeEnum');
    }
    return result;
  }

  static TaiutcIngestDataModeEnum? _createOrNull(int? value) => 
      value == null ? null : TaiutcIngestDataModeEnum.fromValue(value);
  ...
  static const TaiutcIngestDataModeEnum REAL = TaiutcIngestDataModeEnum._(0);

  ///  Specific datasets used to evaluate compliance with specifications and requirements, and for validating technical, functional, and performance characteristics.
  static const TaiutcIngestDataModeEnum TEST = TaiutcIngestDataModeEnum._(1);

  ///  Synthetic data generated by a model to mimic real-world datasets.
  static const TaiutcIngestDataModeEnum SIMULATED = TaiutcIngestDataModeEnum._(2);

  ///  Data pertaining to a government or military exercise. The data may include both real and simulated data.
  static const TaiutcIngestDataModeEnum EXERCISE = TaiutcIngestDataModeEnum._(3);
  static const Map<int, TaiutcIngestDataModeEnum> values = {
    0: REAL,
    1: TEST,
    2: SIMULATED,
    3: EXERCISE};

  static const fb.Reader<TaiutcIngestDataModeEnum> reader = _TaiutcIngestDataModeEnumReader();

  @override
  String toString() {
    return 'TaiutcIngestDataModeEnum{value: $value}';
  }
}

class _TaiutcIngestDataModeEnumReader extends fb.Reader<TaiutcIngestDataModeEnum> {
  const _TaiutcIngestDataModeEnumReader();

  @override
  int get size => 1;

  @override
  TaiutcIngestDataModeEnum read(fb.BufferContext bc, int offset) =>
      TaiutcIngestDataModeEnum.fromValue(const fb.Int8Reader().read(bc, offset));
}
...
class TaiutcIngest {
  TaiutcIngest._(this._bc, this._bcOffset);
  factory TaiutcIngest(List<int> bytes) {
    final rootRef = fb.BufferContext.fromBytes(bytes);
    return reader.read(rootRef, 0);
  }

  static const fb.Reader<TaiutcIngest> reader = _TaiutcIngestReader();
  ...
  ///  Constraints: Minimum length = 1, Maximum length = 32
  TaiutcIngestDataModeEnum get dataMode => TaiutcIngestDataModeEnum.fromValue(const fb.Int8Reader().vTableGet(_bc, _bcOffset, 22, 0));
  ...
  @override
  String toString() {
    return 'TaiutcIngest{id: ${id}, classificationMarking: ${classificationMarking}, adjustmentDate: ${adjustmentDate}, taiUtc: ${taiUtc}, multiplicationFactor: ${multiplicationFactor}, rawFileUri: ${rawFileUri}, source: ${source}, origin: ${origin}, origNetwork: ${origNetwork}, dataMode: ${dataMode}, createdAt: ${createdAt}, createdBy: ${createdBy}}';
  }
}

class _TaiutcIngestReader extends fb.TableReader<TaiutcIngest> {
  const _TaiutcIngestReader();

  @override
  TaiutcIngest createObject(fb.BufferContext bc, int offset) => 
    TaiutcIngest._(bc, offset);
}

class TaiutcIngestBuilder {
  TaiutcIngestBuilder(this.fbBuilder);
  ...
  int addDataMode(TaiutcIngestDataModeEnum? dataMode) {
...
class TaiutcIngestObjectBuilder extends fb.ObjectBuilder {
  final String? _id;
  ...
  final TaiutcIngestDataModeEnum? _dataMode;
  final String? _createdAt;
  final String? _createdBy;

  TaiutcIngestObjectBuilder({
    String? id,
    ...
    String? origin,
    String? origNetwork,
    TaiutcIngestDataModeEnum? dataMode,
    String? createdAt,
    String? createdBy,
  })
      : _id = id,
