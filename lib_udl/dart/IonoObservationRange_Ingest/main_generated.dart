...
class IonoObservationRangeIngest {
  IonoObservationRangeIngest._(this._bc, this._bcOffset);
  factory IonoObservationRangeIngest(List<int> bytes) {
    final rootRef = fb.BufferContext.fromBytes(bytes);
    return reader.read(rootRef, 0);
  }

  static const fb.Reader<IonoObservationRangeIngest> reader = _IonoObservationRangeIngestReader();
  ...
  @override
  String toString() {
    return 'IonoObservationRangeIngest{notes: ${notes}, dimensions: ${dimensions}, dimensionName: ${dimensionName}, data: ${data}}';
  }
}

class _IonoObservationRangeIngestReader extends fb.TableReader<IonoObservationRangeIngest> {
  const _IonoObservationRangeIngestReader();

  @override
  IonoObservationRangeIngest createObject(fb.BufferContext bc, int offset) => 
    IonoObservationRangeIngest._(bc, offset);
}

class IonoObservationRangeIngestBuilder {
  IonoObservationRangeIngestBuilder(this.fbBuilder);
...
class IonoObservationRangeIngestObjectBuilder extends fb.ObjectBuilder {
  final String? _notes;
  final List<String>? _dimensions;
  final List<String>? _dimensionName;
  final List<String>? _data;

  IonoObservationRangeIngestObjectBuilder({
    String? notes,
