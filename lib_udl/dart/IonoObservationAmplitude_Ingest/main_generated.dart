...
class IonoObservationAmplitudeIngest {
  IonoObservationAmplitudeIngest._(this._bc, this._bcOffset);
  factory IonoObservationAmplitudeIngest(List<int> bytes) {
    final rootRef = fb.BufferContext.fromBytes(bytes);
    return reader.read(rootRef, 0);
  }

  static const fb.Reader<IonoObservationAmplitudeIngest> reader = _IonoObservationAmplitudeIngestReader();
  ...
  @override
  String toString() {
    return 'IonoObservationAmplitudeIngest{notes: ${notes}, dimensions: ${dimensions}, dimensionName: ${dimensionName}, data: ${data}}';
  }
}

class _IonoObservationAmplitudeIngestReader extends fb.TableReader<IonoObservationAmplitudeIngest> {
  const _IonoObservationAmplitudeIngestReader();

  @override
  IonoObservationAmplitudeIngest createObject(fb.BufferContext bc, int offset) => 
    IonoObservationAmplitudeIngest._(bc, offset);
}

class IonoObservationAmplitudeIngestBuilder {
  IonoObservationAmplitudeIngestBuilder(this.fbBuilder);
...
class IonoObservationAmplitudeIngestObjectBuilder extends fb.ObjectBuilder {
  final String? _notes;
  final List<String>? _dimensions;
  final List<String>? _dimensionName;
  final List<String>? _data;

  IonoObservationAmplitudeIngestObjectBuilder({
    String? notes,
