...
class IonoObservationTimeFull {
  IonoObservationTimeFull._(this._bc, this._bcOffset);
  factory IonoObservationTimeFull(List<int> bytes) {
    final rootRef = fb.BufferContext.fromBytes(bytes);
    return reader.read(rootRef, 0);
  }

  static const fb.Reader<IonoObservationTimeFull> reader = _IonoObservationTimeFullReader();
  ...
  @override
  String toString() {
    return 'IonoObservationTimeFull{dimensions: ${dimensions}, dimensionName: ${dimensionName}, notes: ${notes}, data: ${data}}';
  }
}

class _IonoObservationTimeFullReader extends fb.TableReader<IonoObservationTimeFull> {
  const _IonoObservationTimeFullReader();

  @override
  IonoObservationTimeFull createObject(fb.BufferContext bc, int offset) => 
    IonoObservationTimeFull._(bc, offset);
}

class IonoObservationTimeFullBuilder {
  IonoObservationTimeFullBuilder(this.fbBuilder);
...
class IonoObservationTimeFullObjectBuilder extends fb.ObjectBuilder {
  final List<String>? _dimensions;
  final List<String>? _dimensionName;
  final String? _notes;
  final List<String>? _data;

  IonoObservationTimeFullObjectBuilder({
