...
class IonoObservationTimeIngest {
  IonoObservationTimeIngest._(this._bc, this._bcOffset);
  factory IonoObservationTimeIngest(List<int> bytes) {
    final rootRef = fb.BufferContext.fromBytes(bytes);
    return reader.read(rootRef, 0);
  }

  static const fb.Reader<IonoObservationTimeIngest> reader = _IonoObservationTimeIngestReader();
  ...
  @override
  String toString() {
    return 'IonoObservationTimeIngest{dimensions: ${dimensions}, dimensionName: ${dimensionName}, notes: ${notes}, data: ${data}}';
  }
}

class _IonoObservationTimeIngestReader extends fb.TableReader<IonoObservationTimeIngest> {
  const _IonoObservationTimeIngestReader();

  @override
  IonoObservationTimeIngest createObject(fb.BufferContext bc, int offset) => 
    IonoObservationTimeIngest._(bc, offset);
}

class IonoObservationTimeIngestBuilder {
  IonoObservationTimeIngestBuilder(this.fbBuilder);
...
class IonoObservationTimeIngestObjectBuilder extends fb.ObjectBuilder {
  final List<String>? _dimensions;
  final List<String>? _dimensionName;
  final String? _notes;
  final List<String>? _data;

  IonoObservationTimeIngestObjectBuilder({
