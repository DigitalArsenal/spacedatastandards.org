...
class IonoObservationScalerInfoIngest {
  IonoObservationScalerInfoIngest._(this._bc, this._bcOffset);
  factory IonoObservationScalerInfoIngest(List<int> bytes) {
    final rootRef = fb.BufferContext.fromBytes(bytes);
    return reader.read(rootRef, 0);
  }

  static const fb.Reader<IonoObservationScalerInfoIngest> reader = _IonoObservationScalerInfoIngestReader();
  ...
  @override
  String toString() {
    return 'IonoObservationScalerInfoIngest{type: ${type}, name: ${name}, organization: ${organization}, version: ${version}, confidenceScore: ${confidenceScore}, confidenceLevel: ${confidenceLevel}}';
  }
}

class _IonoObservationScalerInfoIngestReader extends fb.TableReader<IonoObservationScalerInfoIngest> {
  const _IonoObservationScalerInfoIngestReader();

  @override
  IonoObservationScalerInfoIngest createObject(fb.BufferContext bc, int offset) => 
    IonoObservationScalerInfoIngest._(bc, offset);
}

class IonoObservationScalerInfoIngestBuilder {
  IonoObservationScalerInfoIngestBuilder(this.fbBuilder);
...
class IonoObservationScalerInfoIngestObjectBuilder extends fb.ObjectBuilder {
  final String? _type;
  final String? _name;
  ...
  IonoObservationScalerInfoIngestObjectBuilder({
    String? type,
    String? name,
    String? organization,
    double? version,
