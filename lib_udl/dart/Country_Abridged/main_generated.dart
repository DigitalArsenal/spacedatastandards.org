...
class CountryAbridgedDataModeEnum {
  final int value;
  const CountryAbridgedDataModeEnum._(this.value);

  factory CountryAbridgedDataModeEnum.fromValue(int value) {
    final result = values[value];
    if (result == null) {
        throw StateError('Invalid value $value for bit flag enum CountryAbridgedDataModeEnum');
    }
    return result;
  }

  static CountryAbridgedDataModeEnum? _createOrNull(int? value) => 
      value == null ? null : CountryAbridgedDataModeEnum.fromValue(value);
  ...
  static const CountryAbridgedDataModeEnum REAL = CountryAbridgedDataModeEnum._(0);

  ///  Specific datasets used to evaluate compliance with specifications and requirements, and for validating technical, functional, and performance characteristics.
  static const CountryAbridgedDataModeEnum TEST = CountryAbridgedDataModeEnum._(1);

  ///  Synthetic data generated by a model to mimic real-world datasets.
  static const CountryAbridgedDataModeEnum SIMULATED = CountryAbridgedDataModeEnum._(2);

  ///  Data pertaining to a government or military exercise. The data may include both real and simulated data.
  static const CountryAbridgedDataModeEnum EXERCISE = CountryAbridgedDataModeEnum._(3);
  static const Map<int, CountryAbridgedDataModeEnum> values = {
    0: REAL,
    1: TEST,
    2: SIMULATED,
    3: EXERCISE};

  static const fb.Reader<CountryAbridgedDataModeEnum> reader = _CountryAbridgedDataModeEnumReader();

  @override
  String toString() {
    return 'CountryAbridgedDataModeEnum{value: $value}';
  }
}

class _CountryAbridgedDataModeEnumReader extends fb.Reader<CountryAbridgedDataModeEnum> {
  const _CountryAbridgedDataModeEnumReader();

  @override
  int get size => 1;

  @override
  CountryAbridgedDataModeEnum read(fb.BufferContext bc, int offset) =>
      CountryAbridgedDataModeEnum.fromValue(const fb.Int8Reader().read(bc, offset));
}

///  /// A Country may represent countries, multi-national consortiums, and international organizations.
class CountryAbridged {
  CountryAbridged._(this._bc, this._bcOffset);
  factory CountryAbridged(List<int> bytes) {
    final rootRef = fb.BufferContext.fromBytes(bytes);
    return reader.read(rootRef, 0);
  }

  static const fb.Reader<CountryAbridged> reader = _CountryAbridgedReader();
  ...
  ///  Constraints: Minimum length = 1, Maximum length = 32
  CountryAbridgedDataModeEnum get dataMode => CountryAbridgedDataModeEnum.fromValue(const fb.Int8Reader().vTableGet(_bc, _bcOffset, 22, 0));

  @override
  String toString() {
    return 'CountryAbridged{code: ${code}, name: ${name}, fipsCode: ${fipsCode}, iso3Code: ${iso3Code}, codeAlt: ${codeAlt}, createdAt: ${createdAt}, createdBy: ${createdBy}, source: ${source}, origNetwork: ${origNetwork}, dataMode: ${dataMode}}';
  }
}

class _CountryAbridgedReader extends fb.TableReader<CountryAbridged> {
  const _CountryAbridgedReader();

  @override
  CountryAbridged createObject(fb.BufferContext bc, int offset) => 
    CountryAbridged._(bc, offset);
}

class CountryAbridgedBuilder {
  CountryAbridgedBuilder(this.fbBuilder);
  ...
  int addDataMode(CountryAbridgedDataModeEnum? dataMode) {
...
class CountryAbridgedObjectBuilder extends fb.ObjectBuilder {
  final String? _code;
  final String? _name;
  ...
  final CountryAbridgedDataModeEnum? _dataMode;

  CountryAbridgedObjectBuilder({
    String? code,
    String? name,
    String? fipsCode,
    String? iso3Code,
    ...
    String? source,
    String? origNetwork,
    CountryAbridgedDataModeEnum? dataMode,
  })
      : _code = code,
        _name = name,
        _fipsCode = fipsCode,
