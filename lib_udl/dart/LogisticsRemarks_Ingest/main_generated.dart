...
class LogisticsRemarksIngest {
  LogisticsRemarksIngest._(this._bc, this._bcOffset);
  factory LogisticsRemarksIngest(List<int> bytes) {
    final rootRef = fb.BufferContext.fromBytes(bytes);
    return reader.read(rootRef, 0);
  }

  static const fb.Reader<LogisticsRemarksIngest> reader = _LogisticsRemarksIngestReader();
  ...
  @override
  String toString() {
    return 'LogisticsRemarksIngest{username: ${username}, remark: ${remark}, lastChanged: ${lastChanged}}';
  }
}

class _LogisticsRemarksIngestReader extends fb.TableReader<LogisticsRemarksIngest> {
  const _LogisticsRemarksIngestReader();

  @override
  LogisticsRemarksIngest createObject(fb.BufferContext bc, int offset) => 
    LogisticsRemarksIngest._(bc, offset);
}

class LogisticsRemarksIngestBuilder {
  LogisticsRemarksIngestBuilder(this.fbBuilder);
...
class LogisticsRemarksIngestObjectBuilder extends fb.ObjectBuilder {
  final String? _username;
  final String? _remark;
  final String? _lastChanged;

  LogisticsRemarksIngestObjectBuilder({
    String? username,
    String? remark,
    String? lastChanged,
  })
