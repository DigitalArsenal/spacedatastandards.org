...
class EvacHealthStateAbridged {
  EvacHealthStateAbridged._(this._bc, this._bcOffset);
  factory EvacHealthStateAbridged(List<int> bytes) {
    final rootRef = fb.BufferContext.fromBytes(bytes);
    return reader.read(rootRef, 0);
  }

  static const fb.Reader<EvacHealthStateAbridged> reader = _EvacHealthStateAbridgedReader();
  ...
  @override
  String toString() {
    return 'EvacHealthStateAbridged{type: ${type}, healthStateCode: ${healthStateCode}, medConfFactor: ${medConfFactor}, time: ${time}}';
  }
}

class _EvacHealthStateAbridgedReader extends fb.TableReader<EvacHealthStateAbridged> {
  const _EvacHealthStateAbridgedReader();

  @override
  EvacHealthStateAbridged createObject(fb.BufferContext bc, int offset) => 
    EvacHealthStateAbridged._(bc, offset);
}

class EvacHealthStateAbridgedBuilder {
  EvacHealthStateAbridgedBuilder(this.fbBuilder);
...
class EvacHealthStateAbridgedObjectBuilder extends fb.ObjectBuilder {
  final String? _type;
  final String? _healthStateCode;
  final int? _medConfFactor;
  final String? _time;

  EvacHealthStateAbridgedObjectBuilder({
    String? type,
