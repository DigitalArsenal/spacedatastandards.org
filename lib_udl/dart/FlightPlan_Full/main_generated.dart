...
class FlightPlanFullDataModeEnum {
  final int value;
  const FlightPlanFullDataModeEnum._(this.value);

  factory FlightPlanFullDataModeEnum.fromValue(int value) {
    final result = values[value];
    if (result == null) {
        throw StateError('Invalid value $value for bit flag enum FlightPlanFullDataModeEnum');
    }
    return result;
  }

  static FlightPlanFullDataModeEnum? _createOrNull(int? value) => 
      value == null ? null : FlightPlanFullDataModeEnum.fromValue(value);
  ...
  static const FlightPlanFullDataModeEnum REAL = FlightPlanFullDataModeEnum._(0);

  ///  Specific datasets used to evaluate compliance with specifications and requirements, and for validating technical, functional, and performance characteristics.
  static const FlightPlanFullDataModeEnum TEST = FlightPlanFullDataModeEnum._(1);

  ///  Synthetic data generated by a model to mimic real-world datasets.
  static const FlightPlanFullDataModeEnum SIMULATED = FlightPlanFullDataModeEnum._(2);

  ///  Data pertaining to a government or military exercise. The data may include both real and simulated data.
  static const FlightPlanFullDataModeEnum EXERCISE = FlightPlanFullDataModeEnum._(3);
  static const Map<int, FlightPlanFullDataModeEnum> values = {
    0: REAL,
    1: TEST,
    2: SIMULATED,
    3: EXERCISE};

  static const fb.Reader<FlightPlanFullDataModeEnum> reader = _FlightPlanFullDataModeEnumReader();

  @override
  String toString() {
    return 'FlightPlanFullDataModeEnum{value: $value}';
  }
}

class _FlightPlanFullDataModeEnumReader extends fb.Reader<FlightPlanFullDataModeEnum> {
  const _FlightPlanFullDataModeEnumReader();

  @override
  int get size => 1;

  @override
  FlightPlanFullDataModeEnum read(fb.BufferContext bc, int offset) =>
      FlightPlanFullDataModeEnum.fromValue(const fb.Int8Reader().read(bc, offset));
}
...
class FlightPlanFull {
  FlightPlanFull._(this._bc, this._bcOffset);
  factory FlightPlanFull(List<int> bytes) {
    final rootRef = fb.BufferContext.fromBytes(bytes);
    return reader.read(rootRef, 0);
  }

  static const fb.Reader<FlightPlanFull> reader = _FlightPlanFullReader();
  ...
  ///  Constraints: Minimum length = 1, Maximum length = 32
  FlightPlanFullDataModeEnum get dataMode => FlightPlanFullDataModeEnum.fromValue(const fb.Int8Reader().vTableGet(_bc, _bcOffset, 186, 0));
...
class _FlightPlanFullReader extends fb.TableReader<FlightPlanFull> {
  const _FlightPlanFullReader();

  @override
  FlightPlanFull createObject(fb.BufferContext bc, int offset) => 
    FlightPlanFull._(bc, offset);
}

class FlightPlanFullBuilder {
  FlightPlanFullBuilder(this.fbBuilder);
  ...
  int addDataMode(FlightPlanFullDataModeEnum? dataMode) {
...
class FlightPlanFullObjectBuilder extends fb.ObjectBuilder {
  final String? _id;
  ...
  final FlightPlanFullDataModeEnum? _dataMode;
  ...
  FlightPlanFullObjectBuilder({
    ...
    String? origin,
    String? origNetwork,
    FlightPlanFullDataModeEnum? dataMode,
