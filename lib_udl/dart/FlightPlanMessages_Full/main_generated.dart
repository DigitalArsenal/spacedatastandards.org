...
class FlightPlanMessagesFull {
  FlightPlanMessagesFull._(this._bc, this._bcOffset);
  factory FlightPlanMessagesFull(List<int> bytes) {
    final rootRef = fb.BufferContext.fromBytes(bytes);
    return reader.read(rootRef, 0);
  }

  static const fb.Reader<FlightPlanMessagesFull> reader = _FlightPlanMessagesFullReader();
  ...
  @override
  String toString() {
    return 'FlightPlanMessagesFull{severity: ${severity}, wpNum: ${wpNum}, routePath: ${routePath}, msgText: ${msgText}}';
  }
}

class _FlightPlanMessagesFullReader extends fb.TableReader<FlightPlanMessagesFull> {
  const _FlightPlanMessagesFullReader();

  @override
  FlightPlanMessagesFull createObject(fb.BufferContext bc, int offset) => 
    FlightPlanMessagesFull._(bc, offset);
}

class FlightPlanMessagesFullBuilder {
  FlightPlanMessagesFullBuilder(this.fbBuilder);
...
class FlightPlanMessagesFullObjectBuilder extends fb.ObjectBuilder {
  final String? _severity;
  final String? _wpNum;
  final String? _routePath;
  final String? _msgText;

  FlightPlanMessagesFullObjectBuilder({
    String? severity,
    String? wpNum,
    String? routePath,
    String? msgText,
