...
class LogisticsStocksAbridged {
  LogisticsStocksAbridged._(this._bc, this._bcOffset);
  factory LogisticsStocksAbridged(List<int> bytes) {
    final rootRef = fb.BufferContext.fromBytes(bytes);
    return reader.read(rootRef, 0);
  }

  static const fb.Reader<LogisticsStocksAbridged> reader = _LogisticsStocksAbridgedReader();
  ...
  @override
  String toString() {
    return 'LogisticsStocksAbridged{sourceIcao: ${sourceIcao}, quantity: ${quantity}, stockCheckTime: ${stockCheckTime}, stockPoc: ${stockPoc}}';
  }
}

class _LogisticsStocksAbridgedReader extends fb.TableReader<LogisticsStocksAbridged> {
  const _LogisticsStocksAbridgedReader();

  @override
  LogisticsStocksAbridged createObject(fb.BufferContext bc, int offset) => 
    LogisticsStocksAbridged._(bc, offset);
}

class LogisticsStocksAbridgedBuilder {
  LogisticsStocksAbridgedBuilder(this.fbBuilder);
...
class LogisticsStocksAbridgedObjectBuilder extends fb.ObjectBuilder {
  final String? _sourceIcao;
  final int? _quantity;
  final String? _stockCheckTime;
  final String? _stockPoc;

  LogisticsStocksAbridgedObjectBuilder({
    String? sourceIcao,
    int? quantity,
