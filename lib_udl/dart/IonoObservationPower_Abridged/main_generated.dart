...
class IonoObservationPowerAbridged {
  IonoObservationPowerAbridged._(this._bc, this._bcOffset);
  factory IonoObservationPowerAbridged(List<int> bytes) {
    final rootRef = fb.BufferContext.fromBytes(bytes);
    return reader.read(rootRef, 0);
  }

  static const fb.Reader<IonoObservationPowerAbridged> reader = _IonoObservationPowerAbridgedReader();
  ...
  @override
  String toString() {
    return 'IonoObservationPowerAbridged{notes: ${notes}, dimensions: ${dimensions}, dimensionName: ${dimensionName}, data: ${data}}';
  }
}

class _IonoObservationPowerAbridgedReader extends fb.TableReader<IonoObservationPowerAbridged> {
  const _IonoObservationPowerAbridgedReader();

  @override
  IonoObservationPowerAbridged createObject(fb.BufferContext bc, int offset) => 
    IonoObservationPowerAbridged._(bc, offset);
}

class IonoObservationPowerAbridgedBuilder {
  IonoObservationPowerAbridgedBuilder(this.fbBuilder);
...
class IonoObservationPowerAbridgedObjectBuilder extends fb.ObjectBuilder {
  final String? _notes;
  final List<String>? _dimensions;
  final List<String>? _dimensionName;
  final List<String>? _data;

  IonoObservationPowerAbridgedObjectBuilder({
    String? notes,
