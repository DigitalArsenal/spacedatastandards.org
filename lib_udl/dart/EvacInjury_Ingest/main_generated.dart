...
///  /// Injury specifics.
class EvacInjuryIngest {
  EvacInjuryIngest._(this._bc, this._bcOffset);
  factory EvacInjuryIngest(List<int> bytes) {
    final rootRef = fb.BufferContext.fromBytes(bytes);
    return reader.read(rootRef, 0);
  }

  static const fb.Reader<EvacInjuryIngest> reader = _EvacInjuryIngestReader();
  ...
  @override
  String toString() {
    return 'EvacInjuryIngest{type: ${type}, bodyPart: ${bodyPart}, time: ${time}, comments: ${comments}}';
  }
}

class _EvacInjuryIngestReader extends fb.TableReader<EvacInjuryIngest> {
  const _EvacInjuryIngestReader();

  @override
  EvacInjuryIngest createObject(fb.BufferContext bc, int offset) => 
    EvacInjuryIngest._(bc, offset);
}

class EvacInjuryIngestBuilder {
  EvacInjuryIngestBuilder(this.fbBuilder);
...
class EvacInjuryIngestObjectBuilder extends fb.ObjectBuilder {
  final String? _type;
  final String? _bodyPart;
  final String? _time;
  final String? _comments;

  EvacInjuryIngestObjectBuilder({
    String? type,
    String? bodyPart,
    String? time,
    String? comments,
