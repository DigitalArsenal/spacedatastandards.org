...
class LogisticsStocksFull {
  LogisticsStocksFull._(this._bc, this._bcOffset);
  factory LogisticsStocksFull(List<int> bytes) {
    final rootRef = fb.BufferContext.fromBytes(bytes);
    return reader.read(rootRef, 0);
  }

  static const fb.Reader<LogisticsStocksFull> reader = _LogisticsStocksFullReader();
  ...
  @override
  String toString() {
    return 'LogisticsStocksFull{sourceIcao: ${sourceIcao}, quantity: ${quantity}, stockCheckTime: ${stockCheckTime}, stockPoc: ${stockPoc}}';
  }
}

class _LogisticsStocksFullReader extends fb.TableReader<LogisticsStocksFull> {
  const _LogisticsStocksFullReader();

  @override
  LogisticsStocksFull createObject(fb.BufferContext bc, int offset) => 
    LogisticsStocksFull._(bc, offset);
}

class LogisticsStocksFullBuilder {
  LogisticsStocksFullBuilder(this.fbBuilder);
...
class LogisticsStocksFullObjectBuilder extends fb.ObjectBuilder {
  final String? _sourceIcao;
  final int? _quantity;
  final String? _stockCheckTime;
  final String? _stockPoc;

  LogisticsStocksFullObjectBuilder({
    String? sourceIcao,
    int? quantity,
