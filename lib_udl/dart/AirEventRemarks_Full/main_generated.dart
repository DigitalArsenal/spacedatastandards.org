...
class AirEventRemarksFull {
  AirEventRemarksFull._(this._bc, this._bcOffset);
  factory AirEventRemarksFull(List<int> bytes) {
    final rootRef = fb.BufferContext.fromBytes(bytes);
    return reader.read(rootRef, 0);
  }

  static const fb.Reader<AirEventRemarksFull> reader = _AirEventRemarksFullReader();
  ...
  @override
  String toString() {
    return 'AirEventRemarksFull{externalRemarkId: ${externalRemarkId}, text: ${text}, date: ${date}, user: ${user}}';
  }
}

class _AirEventRemarksFullReader extends fb.TableReader<AirEventRemarksFull> {
  const _AirEventRemarksFullReader();

  @override
  AirEventRemarksFull createObject(fb.BufferContext bc, int offset) => 
    AirEventRemarksFull._(bc, offset);
}

class AirEventRemarksFullBuilder {
  AirEventRemarksFullBuilder(this.fbBuilder);
...
class AirEventRemarksFullObjectBuilder extends fb.ObjectBuilder {
  final String? _externalRemarkId;
  final String? _text;
  final String? _date;
  final String? _user;

  AirEventRemarksFullObjectBuilder({
