...
class VesselIngestDataModeEnum {
  final int value;
  const VesselIngestDataModeEnum._(this.value);

  factory VesselIngestDataModeEnum.fromValue(int value) {
    final result = values[value];
    if (result == null) {
        throw StateError('Invalid value $value for bit flag enum VesselIngestDataModeEnum');
    }
    return result;
  }

  static VesselIngestDataModeEnum? _createOrNull(int? value) => 
      value == null ? null : VesselIngestDataModeEnum.fromValue(value);
  ...
  static const VesselIngestDataModeEnum REAL = VesselIngestDataModeEnum._(0);

  ///  Specific datasets used to evaluate compliance with specifications and requirements, and for validating technical, functional, and performance characteristics.
  static const VesselIngestDataModeEnum TEST = VesselIngestDataModeEnum._(1);

  ///  Synthetic data generated by a model to mimic real-world datasets.
  static const VesselIngestDataModeEnum SIMULATED = VesselIngestDataModeEnum._(2);

  ///  Data pertaining to a government or military exercise. The data may include both real and simulated data.
  static const VesselIngestDataModeEnum EXERCISE = VesselIngestDataModeEnum._(3);
  static const Map<int, VesselIngestDataModeEnum> values = {
    0: REAL,
    1: TEST,
    2: SIMULATED,
    3: EXERCISE};

  static const fb.Reader<VesselIngestDataModeEnum> reader = _VesselIngestDataModeEnumReader();

  @override
  String toString() {
    return 'VesselIngestDataModeEnum{value: $value}';
  }
}

class _VesselIngestDataModeEnumReader extends fb.Reader<VesselIngestDataModeEnum> {
  const _VesselIngestDataModeEnumReader();

  @override
  int get size => 1;

  @override
  VesselIngestDataModeEnum read(fb.BufferContext bc, int offset) =>
      VesselIngestDataModeEnum.fromValue(const fb.Int8Reader().read(bc, offset));
}
...
class VesselIngest {
  VesselIngest._(this._bc, this._bcOffset);
  factory VesselIngest(List<int> bytes) {
    final rootRef = fb.BufferContext.fromBytes(bytes);
    return reader.read(rootRef, 0);
  }

  static const fb.Reader<VesselIngest> reader = _VesselIngestReader();
  ...
  ///  Constraints: Minimum length = 1, Maximum length = 64
  VesselIngestDataModeEnum get dataMode => VesselIngestDataModeEnum.fromValue(const fb.Int8Reader().vTableGet(_bc, _bcOffset, 70, 0));
...
class VesselIngestBuilder {
  ...
  int addDataMode(VesselIngestDataModeEnum? dataMode) {
...
class VesselIngestObjectBuilder extends fb.ObjectBuilder {
  final String? _id;
  ...
  final VesselIngestDataModeEnum? _dataMode;
  final String? _origNetwork;

  VesselIngestObjectBuilder({
    String? id,
    ...
    String? source,
    String? origin,
    VesselIngestDataModeEnum? dataMode,
    String? origNetwork,
  })
      : _id = id,
