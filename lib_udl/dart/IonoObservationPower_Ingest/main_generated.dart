...
class IonoObservationPowerIngest {
  IonoObservationPowerIngest._(this._bc, this._bcOffset);
  factory IonoObservationPowerIngest(List<int> bytes) {
    final rootRef = fb.BufferContext.fromBytes(bytes);
    return reader.read(rootRef, 0);
  }

  static const fb.Reader<IonoObservationPowerIngest> reader = _IonoObservationPowerIngestReader();
  ...
  @override
  String toString() {
    return 'IonoObservationPowerIngest{notes: ${notes}, dimensions: ${dimensions}, dimensionName: ${dimensionName}, data: ${data}}';
  }
}

class _IonoObservationPowerIngestReader extends fb.TableReader<IonoObservationPowerIngest> {
  const _IonoObservationPowerIngestReader();

  @override
  IonoObservationPowerIngest createObject(fb.BufferContext bc, int offset) => 
    IonoObservationPowerIngest._(bc, offset);
}

class IonoObservationPowerIngestBuilder {
  IonoObservationPowerIngestBuilder(this.fbBuilder);
...
class IonoObservationPowerIngestObjectBuilder extends fb.ObjectBuilder {
  final String? _notes;
  final List<String>? _dimensions;
  final List<String>? _dimensionName;
  final List<String>? _data;

  IonoObservationPowerIngestObjectBuilder({
    String? notes,
