...
class IonoObservationScalerInfoAbridged {
  IonoObservationScalerInfoAbridged._(this._bc, this._bcOffset);
  factory IonoObservationScalerInfoAbridged(List<int> bytes) {
    final rootRef = fb.BufferContext.fromBytes(bytes);
    return reader.read(rootRef, 0);
  }

  static const fb.Reader<IonoObservationScalerInfoAbridged> reader = _IonoObservationScalerInfoAbridgedReader();
  ...
  @override
  String toString() {
    return 'IonoObservationScalerInfoAbridged{type: ${type}, name: ${name}, organization: ${organization}, version: ${version}, confidenceScore: ${confidenceScore}, confidenceLevel: ${confidenceLevel}}';
  }
}

class _IonoObservationScalerInfoAbridgedReader extends fb.TableReader<IonoObservationScalerInfoAbridged> {
  const _IonoObservationScalerInfoAbridgedReader();

  @override
  IonoObservationScalerInfoAbridged createObject(fb.BufferContext bc, int offset) => 
    IonoObservationScalerInfoAbridged._(bc, offset);
}

class IonoObservationScalerInfoAbridgedBuilder {
  IonoObservationScalerInfoAbridgedBuilder(this.fbBuilder);
...
class IonoObservationScalerInfoAbridgedObjectBuilder extends fb.ObjectBuilder {
  final String? _type;
  final String? _name;
  ...
  IonoObservationScalerInfoAbridgedObjectBuilder({
    String? type,
    String? name,
    String? organization,
    double? version,
