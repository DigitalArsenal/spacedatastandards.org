...
class IonoObservationFrequencyAbridged {
  IonoObservationFrequencyAbridged._(this._bc, this._bcOffset);
  factory IonoObservationFrequencyAbridged(List<int> bytes) {
    final rootRef = fb.BufferContext.fromBytes(bytes);
    return reader.read(rootRef, 0);
  }

  static const fb.Reader<IonoObservationFrequencyAbridged> reader = _IonoObservationFrequencyAbridgedReader();
  ...
  @override
  String toString() {
    return 'IonoObservationFrequencyAbridged{notes: ${notes}, dimensions: ${dimensions}, dimensionName: ${dimensionName}, data: ${data}}';
  }
}

class _IonoObservationFrequencyAbridgedReader extends fb.TableReader<IonoObservationFrequencyAbridged> {
  const _IonoObservationFrequencyAbridgedReader();

  @override
  IonoObservationFrequencyAbridged createObject(fb.BufferContext bc, int offset) => 
    IonoObservationFrequencyAbridged._(bc, offset);
}

class IonoObservationFrequencyAbridgedBuilder {
  IonoObservationFrequencyAbridgedBuilder(this.fbBuilder);
...
class IonoObservationFrequencyAbridgedObjectBuilder extends fb.ObjectBuilder {
  final String? _notes;
  final List<String>? _dimensions;
  final List<String>? _dimensionName;
  final List<String>? _data;

  IonoObservationFrequencyAbridgedObjectBuilder({
    String? notes,
