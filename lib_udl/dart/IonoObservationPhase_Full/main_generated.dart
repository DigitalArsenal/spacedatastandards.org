...
class IonoObservationPhaseFull {
  IonoObservationPhaseFull._(this._bc, this._bcOffset);
  factory IonoObservationPhaseFull(List<int> bytes) {
    final rootRef = fb.BufferContext.fromBytes(bytes);
    return reader.read(rootRef, 0);
  }

  static const fb.Reader<IonoObservationPhaseFull> reader = _IonoObservationPhaseFullReader();
  ...
  @override
  String toString() {
    return 'IonoObservationPhaseFull{notes: ${notes}, dimensions: ${dimensions}, dimensionName: ${dimensionName}, data: ${data}}';
  }
}

class _IonoObservationPhaseFullReader extends fb.TableReader<IonoObservationPhaseFull> {
  const _IonoObservationPhaseFullReader();

  @override
  IonoObservationPhaseFull createObject(fb.BufferContext bc, int offset) => 
    IonoObservationPhaseFull._(bc, offset);
}

class IonoObservationPhaseFullBuilder {
  IonoObservationPhaseFullBuilder(this.fbBuilder);
...
class IonoObservationPhaseFullObjectBuilder extends fb.ObjectBuilder {
  final String? _notes;
  final List<String>? _dimensions;
  final List<String>? _dimensionName;
  final List<String>? _data;

  IonoObservationPhaseFullObjectBuilder({
    String? notes,
