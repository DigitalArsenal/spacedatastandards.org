...
class FlightPlanMessagesIngest {
  FlightPlanMessagesIngest._(this._bc, this._bcOffset);
  factory FlightPlanMessagesIngest(List<int> bytes) {
    final rootRef = fb.BufferContext.fromBytes(bytes);
    return reader.read(rootRef, 0);
  }

  static const fb.Reader<FlightPlanMessagesIngest> reader = _FlightPlanMessagesIngestReader();
  ...
  @override
  String toString() {
    return 'FlightPlanMessagesIngest{severity: ${severity}, wpNum: ${wpNum}, routePath: ${routePath}, msgText: ${msgText}}';
  }
}

class _FlightPlanMessagesIngestReader extends fb.TableReader<FlightPlanMessagesIngest> {
  const _FlightPlanMessagesIngestReader();

  @override
  FlightPlanMessagesIngest createObject(fb.BufferContext bc, int offset) => 
    FlightPlanMessagesIngest._(bc, offset);
}

class FlightPlanMessagesIngestBuilder {
  FlightPlanMessagesIngestBuilder(this.fbBuilder);
...
class FlightPlanMessagesIngestObjectBuilder extends fb.ObjectBuilder {
  final String? _severity;
  final String? _wpNum;
  final String? _routePath;
  final String? _msgText;

  FlightPlanMessagesIngestObjectBuilder({
    String? severity,
    String? wpNum,
    String? routePath,
    String? msgText,
