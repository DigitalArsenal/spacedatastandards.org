...
class LogisticsPartsFull {
  LogisticsPartsFull._(this._bc, this._bcOffset);
  factory LogisticsPartsFull(List<int> bytes) {
    final rootRef = fb.BufferContext.fromBytes(bytes);
    return reader.read(rootRef, 0);
  }

  static const fb.Reader<LogisticsPartsFull> reader = _LogisticsPartsFullReader();
  ...
  @override
  String toString() {
    return 'LogisticsPartsFull{partNumber: ${partNumber}, nationalStockNumber: ${nationalStockNumber}, measurementUnitCode: ${measurementUnitCode}, workUnitCode: ${workUnitCode}, technicalOrderText: ${technicalOrderText}, figureNumber: ${figureNumber}, indexNumber: ${indexNumber}, requestVerifier: ${requestVerifier}, locationVerifier: ${locationVerifier}, supplyDocumentNumber: ${supplyDocumentNumber}, logisticsStocks: ${logisticsStocks}}';
  }
}

class _LogisticsPartsFullReader extends fb.TableReader<LogisticsPartsFull> {
  const _LogisticsPartsFullReader();

  @override
  LogisticsPartsFull createObject(fb.BufferContext bc, int offset) => 
    LogisticsPartsFull._(bc, offset);
}

class LogisticsPartsFullBuilder {
  LogisticsPartsFullBuilder(this.fbBuilder);
...
class LogisticsPartsFullObjectBuilder extends fb.ObjectBuilder {
  ...
  LogisticsPartsFullObjectBuilder({
    String? partNumber,
