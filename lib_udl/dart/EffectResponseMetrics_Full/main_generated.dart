...
class EffectResponseMetricsFull {
  EffectResponseMetricsFull._(this._bc, this._bcOffset);
  factory EffectResponseMetricsFull(List<int> bytes) {
    final rootRef = fb.BufferContext.fromBytes(bytes);
    return reader.read(rootRef, 0);
  }

  static const fb.Reader<EffectResponseMetricsFull> reader = _EffectResponseMetricsFullReader();
  ...
  @override
  String toString() {
    return 'EffectResponseMetricsFull{metricType: ${metricType}, domainValue: ${domainValue}, relativeValue: ${relativeValue}, provenance: ${provenance}}';
  }
}

class _EffectResponseMetricsFullReader extends fb.TableReader<EffectResponseMetricsFull> {
  const _EffectResponseMetricsFullReader();

  @override
  EffectResponseMetricsFull createObject(fb.BufferContext bc, int offset) => 
    EffectResponseMetricsFull._(bc, offset);
}

class EffectResponseMetricsFullBuilder {
  EffectResponseMetricsFullBuilder(this.fbBuilder);
...
class EffectResponseMetricsFullObjectBuilder extends fb.ObjectBuilder {
  final String? _metricType;
  final double? _domainValue;
  final double? _relativeValue;
  final String? _provenance;

  EffectResponseMetricsFullObjectBuilder({
    String? metricType,
    double? domainValue,
