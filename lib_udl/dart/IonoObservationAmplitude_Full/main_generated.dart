...
class IonoObservationAmplitudeFull {
  IonoObservationAmplitudeFull._(this._bc, this._bcOffset);
  factory IonoObservationAmplitudeFull(List<int> bytes) {
    final rootRef = fb.BufferContext.fromBytes(bytes);
    return reader.read(rootRef, 0);
  }

  static const fb.Reader<IonoObservationAmplitudeFull> reader = _IonoObservationAmplitudeFullReader();
  ...
  @override
  String toString() {
    return 'IonoObservationAmplitudeFull{notes: ${notes}, dimensions: ${dimensions}, dimensionName: ${dimensionName}, data: ${data}}';
  }
}

class _IonoObservationAmplitudeFullReader extends fb.TableReader<IonoObservationAmplitudeFull> {
  const _IonoObservationAmplitudeFullReader();

  @override
  IonoObservationAmplitudeFull createObject(fb.BufferContext bc, int offset) => 
    IonoObservationAmplitudeFull._(bc, offset);
}

class IonoObservationAmplitudeFullBuilder {
  IonoObservationAmplitudeFullBuilder(this.fbBuilder);
...
class IonoObservationAmplitudeFullObjectBuilder extends fb.ObjectBuilder {
  final String? _notes;
  final List<String>? _dimensions;
  final List<String>? _dimensionName;
  final List<String>? _data;

  IonoObservationAmplitudeFullObjectBuilder({
    String? notes,
