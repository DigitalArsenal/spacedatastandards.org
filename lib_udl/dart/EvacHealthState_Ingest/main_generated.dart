...
class EvacHealthStateIngest {
  EvacHealthStateIngest._(this._bc, this._bcOffset);
  factory EvacHealthStateIngest(List<int> bytes) {
    final rootRef = fb.BufferContext.fromBytes(bytes);
    return reader.read(rootRef, 0);
  }

  static const fb.Reader<EvacHealthStateIngest> reader = _EvacHealthStateIngestReader();
  ...
  @override
  String toString() {
    return 'EvacHealthStateIngest{type: ${type}, healthStateCode: ${healthStateCode}, medConfFactor: ${medConfFactor}, time: ${time}}';
  }
}

class _EvacHealthStateIngestReader extends fb.TableReader<EvacHealthStateIngest> {
  const _EvacHealthStateIngestReader();

  @override
  EvacHealthStateIngest createObject(fb.BufferContext bc, int offset) => 
    EvacHealthStateIngest._(bc, offset);
}

class EvacHealthStateIngestBuilder {
  EvacHealthStateIngestBuilder(this.fbBuilder);
...
class EvacHealthStateIngestObjectBuilder extends fb.ObjectBuilder {
  final String? _type;
  final String? _healthStateCode;
  final int? _medConfFactor;
  final String? _time;

  EvacHealthStateIngestObjectBuilder({
    String? type,
