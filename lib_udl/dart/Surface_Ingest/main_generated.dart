...
class SurfaceIngestDataModeEnum {
  final int value;
  const SurfaceIngestDataModeEnum._(this.value);

  factory SurfaceIngestDataModeEnum.fromValue(int value) {
    final result = values[value];
    if (result == null) {
        throw StateError('Invalid value $value for bit flag enum SurfaceIngestDataModeEnum');
    }
    return result;
  }

  static SurfaceIngestDataModeEnum? _createOrNull(int? value) => 
      value == null ? null : SurfaceIngestDataModeEnum.fromValue(value);
  ...
  static const SurfaceIngestDataModeEnum REAL = SurfaceIngestDataModeEnum._(0);

  ///  No description available.
  static const SurfaceIngestDataModeEnum TEST = SurfaceIngestDataModeEnum._(1);

  ///  No description available.
  static const SurfaceIngestDataModeEnum SIMULATED = SurfaceIngestDataModeEnum._(2);

  ///  No description available.
  static const SurfaceIngestDataModeEnum EXERCISE = SurfaceIngestDataModeEnum._(3);
  static const Map<int, SurfaceIngestDataModeEnum> values = {
    0: REAL,
    1: TEST,
    2: SIMULATED,
    3: EXERCISE};

  static const fb.Reader<SurfaceIngestDataModeEnum> reader = _SurfaceIngestDataModeEnumReader();

  @override
  String toString() {
    return 'SurfaceIngestDataModeEnum{value: $value}';
  }
}

class _SurfaceIngestDataModeEnumReader extends fb.Reader<SurfaceIngestDataModeEnum> {
  const _SurfaceIngestDataModeEnumReader();

  @override
  int get size => 1;

  @override
  SurfaceIngestDataModeEnum read(fb.BufferContext bc, int offset) =>
      SurfaceIngestDataModeEnum.fromValue(const fb.Int8Reader().read(bc, offset));
}
...
class SurfaceIngest {
  SurfaceIngest._(this._bc, this._bcOffset);
  factory SurfaceIngest(List<int> bytes) {
    final rootRef = fb.BufferContext.fromBytes(bytes);
    return reader.read(rootRef, 0);
  }

  static const fb.Reader<SurfaceIngest> reader = _SurfaceIngestReader();
...
class SurfaceIngestBuilder {
  ...
  int addDataMode(SurfaceIngestDataModeEnum? dataMode) {
...
class SurfaceIngestObjectBuilder extends fb.ObjectBuilder {
  final String? _id;
  ...
  final SurfaceIngestDataModeEnum? _dataMode;

  SurfaceIngestObjectBuilder({
    String? id,
    ...
    String? origin,
    String? origNetwork,
    SurfaceIngestDataModeEnum? dataMode,
  })
      : _id = id,
