...
class SensorIngestDataModeEnum {
  final int value;
  const SensorIngestDataModeEnum._(this.value);

  factory SensorIngestDataModeEnum.fromValue(int value) {
    final result = values[value];
    if (result == null) {
        throw StateError('Invalid value $value for bit flag enum SensorIngestDataModeEnum');
    }
    return result;
  }

  static SensorIngestDataModeEnum? _createOrNull(int? value) => 
      value == null ? null : SensorIngestDataModeEnum.fromValue(value);
  ...
  static const SensorIngestDataModeEnum REAL = SensorIngestDataModeEnum._(0);

  ///  Specific datasets used to evaluate compliance with specifications and requirements, and for validating technical, functional, and performance characteristics.
  static const SensorIngestDataModeEnum TEST = SensorIngestDataModeEnum._(1);

  ///  Synthetic data generated by a model to mimic real-world datasets.
  static const SensorIngestDataModeEnum SIMULATED = SensorIngestDataModeEnum._(2);

  ///  Data pertaining to a government or military exercise. The data may include both real and simulated data.
  static const SensorIngestDataModeEnum EXERCISE = SensorIngestDataModeEnum._(3);
  static const Map<int, SensorIngestDataModeEnum> values = {
    0: REAL,
    1: TEST,
    2: SIMULATED,
    3: EXERCISE};

  static const fb.Reader<SensorIngestDataModeEnum> reader = _SensorIngestDataModeEnumReader();

  @override
  String toString() {
    return 'SensorIngestDataModeEnum{value: $value}';
  }
}

class _SensorIngestDataModeEnumReader extends fb.Reader<SensorIngestDataModeEnum> {
  const _SensorIngestDataModeEnumReader();

  @override
  int get size => 1;

  @override
  SensorIngestDataModeEnum read(fb.BufferContext bc, int offset) =>
      SensorIngestDataModeEnum.fromValue(const fb.Int8Reader().read(bc, offset));
}

///  /// Model representation of observation data for electro-optical based sensor phenomenologies.
class SensorIngest {
  SensorIngest._(this._bc, this._bcOffset);
  factory SensorIngest(List<int> bytes) {
    final rootRef = fb.BufferContext.fromBytes(bytes);
    return reader.read(rootRef, 0);
  }

  static const fb.Reader<SensorIngest> reader = _SensorIngestReader();
  ...
  ///  Constraints: Minimum length = 1, Maximum length = 32
  SensorIngestDataModeEnum get dataMode => SensorIngestDataModeEnum.fromValue(const fb.Int8Reader().vTableGet(_bc, _bcOffset, 26, 0));
...
class SensorIngestBuilder {
  ...
  int addDataMode(SensorIngestDataModeEnum? dataMode) {
...
class SensorIngestObjectBuilder extends fb.ObjectBuilder {
  ...
  final SensorIngestDataModeEnum? _dataMode;
  ...
  SensorIngestObjectBuilder({
    ...
    String? origin,
    String? origNetwork,
    SensorIngestDataModeEnum? dataMode,
    String? createdAt,
    String? createdBy,
    bool? active,
