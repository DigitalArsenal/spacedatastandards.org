...
class IonoObservationPowerFull {
  IonoObservationPowerFull._(this._bc, this._bcOffset);
  factory IonoObservationPowerFull(List<int> bytes) {
    final rootRef = fb.BufferContext.fromBytes(bytes);
    return reader.read(rootRef, 0);
  }

  static const fb.Reader<IonoObservationPowerFull> reader = _IonoObservationPowerFullReader();
  ...
  @override
  String toString() {
    return 'IonoObservationPowerFull{notes: ${notes}, dimensions: ${dimensions}, dimensionName: ${dimensionName}, data: ${data}}';
  }
}

class _IonoObservationPowerFullReader extends fb.TableReader<IonoObservationPowerFull> {
  const _IonoObservationPowerFullReader();

  @override
  IonoObservationPowerFull createObject(fb.BufferContext bc, int offset) => 
    IonoObservationPowerFull._(bc, offset);
}

class IonoObservationPowerFullBuilder {
  IonoObservationPowerFullBuilder(this.fbBuilder);
...
class IonoObservationPowerFullObjectBuilder extends fb.ObjectBuilder {
  final String? _notes;
  final List<String>? _dimensions;
  final List<String>? _dimensionName;
  final List<String>? _data;

  IonoObservationPowerFullObjectBuilder({
    String? notes,
