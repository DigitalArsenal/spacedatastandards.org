...
///  /// Array of Doppler.
class IonoObservationDopplerIngest {
  IonoObservationDopplerIngest._(this._bc, this._bcOffset);
  factory IonoObservationDopplerIngest(List<int> bytes) {
    final rootRef = fb.BufferContext.fromBytes(bytes);
    return reader.read(rootRef, 0);
  }

  static const fb.Reader<IonoObservationDopplerIngest> reader = _IonoObservationDopplerIngestReader();
  ...
  @override
  String toString() {
    return 'IonoObservationDopplerIngest{notes: ${notes}, dimensions: ${dimensions}, dimensionName: ${dimensionName}, data: ${data}}';
  }
}

class _IonoObservationDopplerIngestReader extends fb.TableReader<IonoObservationDopplerIngest> {
  const _IonoObservationDopplerIngestReader();

  @override
  IonoObservationDopplerIngest createObject(fb.BufferContext bc, int offset) => 
    IonoObservationDopplerIngest._(bc, offset);
}

class IonoObservationDopplerIngestBuilder {
  IonoObservationDopplerIngestBuilder(this.fbBuilder);
...
class IonoObservationDopplerIngestObjectBuilder extends fb.ObjectBuilder {
  final String? _notes;
  final List<String>? _dimensions;
  final List<String>? _dimensionName;
  final List<String>? _data;

  IonoObservationDopplerIngestObjectBuilder({
    String? notes,
