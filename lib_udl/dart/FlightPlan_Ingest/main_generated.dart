...
class FlightPlanIngestDataModeEnum {
  final int value;
  const FlightPlanIngestDataModeEnum._(this.value);

  factory FlightPlanIngestDataModeEnum.fromValue(int value) {
    final result = values[value];
    if (result == null) {
        throw StateError('Invalid value $value for bit flag enum FlightPlanIngestDataModeEnum');
    }
    return result;
  }

  static FlightPlanIngestDataModeEnum? _createOrNull(int? value) => 
      value == null ? null : FlightPlanIngestDataModeEnum.fromValue(value);
  ...
  static const FlightPlanIngestDataModeEnum REAL = FlightPlanIngestDataModeEnum._(0);

  ///  Specific datasets used to evaluate compliance with specifications and requirements, and for validating technical, functional, and performance characteristics.
  static const FlightPlanIngestDataModeEnum TEST = FlightPlanIngestDataModeEnum._(1);

  ///  Synthetic data generated by a model to mimic real-world datasets.
  static const FlightPlanIngestDataModeEnum SIMULATED = FlightPlanIngestDataModeEnum._(2);

  ///  Data pertaining to a government or military exercise. The data may include both real and simulated data.
  static const FlightPlanIngestDataModeEnum EXERCISE = FlightPlanIngestDataModeEnum._(3);
  static const Map<int, FlightPlanIngestDataModeEnum> values = {
    0: REAL,
    1: TEST,
    2: SIMULATED,
    3: EXERCISE};

  static const fb.Reader<FlightPlanIngestDataModeEnum> reader = _FlightPlanIngestDataModeEnumReader();

  @override
  String toString() {
    return 'FlightPlanIngestDataModeEnum{value: $value}';
  }
}

class _FlightPlanIngestDataModeEnumReader extends fb.Reader<FlightPlanIngestDataModeEnum> {
  const _FlightPlanIngestDataModeEnumReader();

  @override
  int get size => 1;

  @override
  FlightPlanIngestDataModeEnum read(fb.BufferContext bc, int offset) =>
      FlightPlanIngestDataModeEnum.fromValue(const fb.Int8Reader().read(bc, offset));
}

///  /// Flight Plan contains data specifying the details of an intended flight including schedule and expected route.
class FlightPlanIngest {
  FlightPlanIngest._(this._bc, this._bcOffset);
  factory FlightPlanIngest(List<int> bytes) {
    final rootRef = fb.BufferContext.fromBytes(bytes);
    return reader.read(rootRef, 0);
  }

  static const fb.Reader<FlightPlanIngest> reader = _FlightPlanIngestReader();
  ...
  ///  Constraints: Minimum length = 1, Maximum length = 32
  FlightPlanIngestDataModeEnum get dataMode => FlightPlanIngestDataModeEnum.fromValue(const fb.Int8Reader().vTableGet(_bc, _bcOffset, 186, 0));
...
class FlightPlanIngestBuilder {
  ...
  int addDataMode(FlightPlanIngestDataModeEnum? dataMode) {
...
class FlightPlanIngestObjectBuilder extends fb.ObjectBuilder {
  final String? _id;
  ...
  final FlightPlanIngestDataModeEnum? _dataMode;
  ...
  FlightPlanIngestObjectBuilder({
    ...
    String? origin,
    String? origNetwork,
    FlightPlanIngestDataModeEnum? dataMode,
