...
public enum AirEvent_Full_dataMode_Enum: Int8, Enum, Verifiable {
  ...
  public static var max: AirEvent_Full_dataMode_Enum { return .exercise }
  public static var min: AirEvent_Full_dataMode_Enum { return .real }
}


///  /// Information related to an air event (e.g. FUEL TRANSFER, AIR DROP) and the associated aircraft.
public struct AirEvent_Full: FlatBufferObject, Verifiable {
  ...
  public static func finish(_ fbb: inout FlatBufferBuilder, end: Offset, prefix: Bool = false) { fbb.finish(offset: end, fileId: AirEvent_Full.id, addPrefix: prefix) }
  ...
  ///  Constraints: Minimum length = 1, Maximum length = 32
  public var dataMode: AirEvent_Full_dataMode_Enum { let o = _accessor.offset(VTOFFSET.dataMode.v); return o == 0 ? .real : AirEvent_Full_dataMode_Enum(rawValue: _accessor.readBuffer(of: Int8.self, at: o)) ?? .real }
  public static func startAirEvent_Full(_ fbb: inout FlatBufferBuilder) -> UOffset { fbb.startTable(with: 44) }
  ...
  public static func add(dataMode: AirEvent_Full_dataMode_Enum, _ fbb: inout FlatBufferBuilder) { fbb.add(element: dataMode.rawValue, def: 0, at: VTOFFSET.dataMode.p) }
  public static func endAirEvent_Full(_ fbb: inout FlatBufferBuilder, start: UOffset) -> Offset { let end = Offset(offset: fbb.endTable(at: start)); return end }
  public static func createAirEvent_Full(
    ...
    let __start = AirEvent_Full.startAirEvent_Full(&fbb)
    AirEvent_Full.add(id: id, &fbb)
    AirEvent_Full.add(classificationMarking: classificationMarking, &fbb)
    AirEvent_Full.add(type: type, &fbb)
    AirEvent_Full.add(idSortie: idSortie, &fbb)
    AirEvent_Full.add(idMission: idMission, &fbb)
    AirEvent_Full.add(externalAirEventId: externalAirEventId, &fbb)
    AirEvent_Full.add(legNum: legNum, &fbb)
    AirEvent_Full.add(location: location, &fbb)
    AirEvent_Full.add(arrPurpose: arrPurpose, &fbb)
    AirEvent_Full.add(plannedArrTime: plannedArrTime, &fbb)
    AirEvent_Full.add(estArrTime: estArrTime, &fbb)
    AirEvent_Full.add(actualArrTime: actualArrTime, &fbb)
    AirEvent_Full.add(depPurpose: depPurpose, &fbb)
    AirEvent_Full.add(plannedDepTime: plannedDepTime, &fbb)
    AirEvent_Full.add(estDepTime: estDepTime, &fbb)
    AirEvent_Full.add(actualDepTime: actualDepTime, &fbb)
    AirEvent_Full.add(externalArtrackId: externalArtrackId, &fbb)
    AirEvent_Full.add(arTrackId: arTrackId, &fbb)
    AirEvent_Full.add(arTrackName: arTrackName, &fbb)
    AirEvent_Full.add(rzct: rzct, &fbb)
    AirEvent_Full.add(arct: arct, &fbb)
    AirEvent_Full.add(baseAlt: baseAlt, &fbb)
    AirEvent_Full.add(priority: priority, &fbb)
    AirEvent_Full.add(trackTime: trackTime, &fbb)
    AirEvent_Full.add(statusCode: statusCode, &fbb)
    AirEvent_Full.add(rzPoint: rzPoint, &fbb)
    AirEvent_Full.add(rzType: rzType, &fbb)
    AirEvent_Full.add(arEventType: arEventType, &fbb)
    AirEvent_Full.add(numTankers: numTankers, &fbb)
    AirEvent_Full.add(revTrack: revTrack, &fbb)
    AirEvent_Full.add(shortTrack: shortTrack, &fbb)
    AirEvent_Full.add(cancelled: cancelled, &fbb)
    AirEvent_Full.addVectorOf(tankers: tankers, &fbb)
    AirEvent_Full.addVectorOf(receivers: receivers, &fbb)
    AirEvent_Full.addVectorOf(remarks: remarks, &fbb)
    AirEvent_Full.add(createdAt: createdAt, &fbb)
    AirEvent_Full.add(createdBy: createdBy, &fbb)
    AirEvent_Full.add(updatedAt: updatedAt, &fbb)
    AirEvent_Full.add(updatedBy: updatedBy, &fbb)
    AirEvent_Full.add(sourceDl: sourceDl, &fbb)
    AirEvent_Full.add(source: source, &fbb)
    AirEvent_Full.add(origin: origin, &fbb)
    AirEvent_Full.add(origNetwork: origNetwork, &fbb)
    AirEvent_Full.add(dataMode: dataMode, &fbb)
    return AirEvent_Full.endAirEvent_Full(&fbb, start: __start)
  }
  ...
  public static func verify<T>(_ verifier: inout Verifier, at position: Int, of type: T.Type) throws where T: Verifiable {
    ...
    try _v.visit(field: VTOFFSET.dataMode.p, fieldName: "dataMode", required: false, type: AirEvent_Full_dataMode_Enum.self)
    _v.finish()
  }
}
