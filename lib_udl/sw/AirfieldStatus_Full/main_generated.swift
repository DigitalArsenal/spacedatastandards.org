...
public enum AirfieldStatus_Full_dataMode_Enum: Int8, Enum, Verifiable {
  ...
  public static var max: AirfieldStatus_Full_dataMode_Enum { return .exercise }
  public static var min: AirfieldStatus_Full_dataMode_Enum { return .real }
}


///  /// The airfield status contains dynamic data of an airfield's capabilities.
public struct AirfieldStatus_Full: FlatBufferObject, Verifiable {
  ...
  public static func finish(_ fbb: inout FlatBufferBuilder, end: Offset, prefix: Bool = false) { fbb.finish(offset: end, fileId: AirfieldStatus_Full.id, addPrefix: prefix) }
  ...
  ///  Constraints: Minimum length = 1, Maximum length = 32
  public var dataMode: AirfieldStatus_Full_dataMode_Enum { let o = _accessor.offset(VTOFFSET.dataMode.v); return o == 0 ? .real : AirfieldStatus_Full_dataMode_Enum(rawValue: _accessor.readBuffer(of: Int8.self, at: o)) ?? .real }
  public static func startAirfieldStatus_Full(_ fbb: inout FlatBufferBuilder) -> UOffset { fbb.startTable(with: 38) }
  ...
  public static func add(dataMode: AirfieldStatus_Full_dataMode_Enum, _ fbb: inout FlatBufferBuilder) { fbb.add(element: dataMode.rawValue, def: 0, at: VTOFFSET.dataMode.p) }
  public static func endAirfieldStatus_Full(_ fbb: inout FlatBufferBuilder, start: UOffset) -> Offset { let end = Offset(offset: fbb.endTable(at: start)); return end }
  public static func createAirfieldStatus_Full(
    ...
    let __start = AirfieldStatus_Full.startAirfieldStatus_Full(&fbb)
    AirfieldStatus_Full.add(id: id, &fbb)
    AirfieldStatus_Full.add(classificationMarking: classificationMarking, &fbb)
    AirfieldStatus_Full.add(idAirfield: idAirfield, &fbb)
    AirfieldStatus_Full.add(altAirfieldId: altAirfieldId, &fbb)
    AirfieldStatus_Full.add(passengerServiceMog: passengerServiceMog, &fbb)
    AirfieldStatus_Full.add(fleetServiceMog: fleetServiceMog, &fbb)
    AirfieldStatus_Full.add(cargoMog: cargoMog, &fbb)
    AirfieldStatus_Full.add(fuelMog: fuelMog, &fbb)
    AirfieldStatus_Full.addVectorOf(fuelTypes: fuelTypes, &fbb)
    AirfieldStatus_Full.addVectorOf(fuelQtys: fuelQtys, &fbb)
    AirfieldStatus_Full.add(mxMog: mxMog, &fbb)
    AirfieldStatus_Full.add(operatingMog: operatingMog, &fbb)
    AirfieldStatus_Full.add(numCog: numCog, &fbb)
    AirfieldStatus_Full.add(wideWorkingMog: wideWorkingMog, &fbb)
    AirfieldStatus_Full.add(wideParkingMog: wideParkingMog, &fbb)
    AirfieldStatus_Full.add(narrowWorkingMog: narrowWorkingMog, &fbb)
    AirfieldStatus_Full.add(narrowParkingMog: narrowParkingMog, &fbb)
    AirfieldStatus_Full.addVectorOf(mheTypes: mheTypes, &fbb)
    AirfieldStatus_Full.addVectorOf(mheQtys: mheQtys, &fbb)
    AirfieldStatus_Full.add(medCap: medCap, &fbb)
    AirfieldStatus_Full.add(arffCat: arffCat, &fbb)
    AirfieldStatus_Full.add(gseTime: gseTime, &fbb)
    AirfieldStatus_Full.add(priRwyNum: priRwyNum, &fbb)
    AirfieldStatus_Full.add(rwyCondReading: rwyCondReading, &fbb)
    AirfieldStatus_Full.add(rwyFrictionFactor: rwyFrictionFactor, &fbb)
    AirfieldStatus_Full.addVectorOf(rwyMarkings: rwyMarkings, &fbb)
    AirfieldStatus_Full.add(message: message, &fbb)
    AirfieldStatus_Full.add(priFreq: priFreq, &fbb)
    AirfieldStatus_Full.addVectorOf(slotTypesReq: slotTypesReq, &fbb)
    AirfieldStatus_Full.add(createdAt: createdAt, &fbb)
    AirfieldStatus_Full.add(createdBy: createdBy, &fbb)
    AirfieldStatus_Full.add(updatedAt: updatedAt, &fbb)
    AirfieldStatus_Full.add(updatedBy: updatedBy, &fbb)
    AirfieldStatus_Full.add(sourceDl: sourceDl, &fbb)
    AirfieldStatus_Full.add(source: source, &fbb)
    AirfieldStatus_Full.add(origin: origin, &fbb)
    AirfieldStatus_Full.add(origNetwork: origNetwork, &fbb)
    AirfieldStatus_Full.add(dataMode: dataMode, &fbb)
    return AirfieldStatus_Full.endAirfieldStatus_Full(&fbb, start: __start)
  }
  ...
  public static func verify<T>(_ verifier: inout Verifier, at position: Int, of type: T.Type) throws where T: Verifiable {
    ...
    try _v.visit(field: VTOFFSET.dataMode.p, fieldName: "dataMode", required: false, type: AirfieldStatus_Full_dataMode_Enum.self)
    _v.finish()
  }
}
