...
public enum Vessel_Full_dataMode_Enum: Int8, Enum, Verifiable {
  ...
  public static var max: Vessel_Full_dataMode_Enum { return .exercise }
  public static var min: Vessel_Full_dataMode_Enum { return .real }
}
...
public struct Vessel_Full: FlatBufferObject, Verifiable {
  ...
  public static func finish(_ fbb: inout FlatBufferBuilder, end: Offset, prefix: Bool = false) { fbb.finish(offset: end, fileId: Vessel_Full.id, addPrefix: prefix) }
  ...
  ///  Constraints: Minimum length = 1, Maximum length = 64
  public var dataMode: Vessel_Full_dataMode_Enum { let o = _accessor.offset(VTOFFSET.dataMode.v); return o == 0 ? .real : Vessel_Full_dataMode_Enum(rawValue: _accessor.readBuffer(of: Int8.self, at: o)) ?? .real }
  ///  The originating source network on which this record was created, auto-populated by the system.
  ///  Example: /// Example: OPS1
  ///  Constraints: Minimum length = 1, Maximum length = 32
  public var origNetwork: String? { let o = _accessor.offset(VTOFFSET.origNetwork.v); return o == 0 ? nil : _accessor.string(at: o) }
  public var origNetworkSegmentArray: [UInt8]? { return _accessor.getVector(at: VTOFFSET.origNetwork.v) }
  public static func startVessel_Full(_ fbb: inout FlatBufferBuilder) -> UOffset { fbb.startTable(with: 38) }
  ...
  public static func add(dataMode: Vessel_Full_dataMode_Enum, _ fbb: inout FlatBufferBuilder) { fbb.add(element: dataMode.rawValue, def: 0, at: VTOFFSET.dataMode.p) }
  public static func add(origNetwork: Offset, _ fbb: inout FlatBufferBuilder) { fbb.add(offset: origNetwork, at: VTOFFSET.origNetwork.p) }
  public static func endVessel_Full(_ fbb: inout FlatBufferBuilder, start: UOffset) -> Offset { let end = Offset(offset: fbb.endTable(at: start)); return end }
  public static func createVessel_Full(
    ...
    let __start = Vessel_Full.startVessel_Full(&fbb)
    Vessel_Full.add(id: id, &fbb)
    Vessel_Full.add(classificationMarking: classificationMarking, &fbb)
    Vessel_Full.add(entity: entity, &fbb)
    Vessel_Full.add(idEntity: idEntity, &fbb)
    Vessel_Full.add(altVesselId: altVesselId, &fbb)
    Vessel_Full.add(idOrganization: idOrganization, &fbb)
    Vessel_Full.add(vesselName: vesselName, &fbb)
    Vessel_Full.add(vesselType: vesselType, &fbb)
    Vessel_Full.add(vesselClass: vesselClass, &fbb)
    Vessel_Full.add(vesselDescription: vesselDescription, &fbb)
    Vessel_Full.add(sconum: sconum, &fbb)
    Vessel_Full.add(imon: imon, &fbb)
    Vessel_Full.add(mmsi: mmsi, &fbb)
    Vessel_Full.add(callsign: callsign, &fbb)
    Vessel_Full.add(vesselFlag: vesselFlag, &fbb)
    Vessel_Full.add(yearBuilt: yearBuilt, &fbb)
    Vessel_Full.add(vesselBuilder: vesselBuilder, &fbb)
    Vessel_Full.add(status: status, &fbb)
    Vessel_Full.add(firstSeen: firstSeen, &fbb)
    Vessel_Full.add(length: length, &fbb)
    Vessel_Full.add(width: width, &fbb)
    Vessel_Full.add(maxDraught: maxDraught, &fbb)
    Vessel_Full.add(vslWt: vslWt, &fbb)
    Vessel_Full.add(maxSpeed: maxSpeed, &fbb)
    Vessel_Full.add(sternType: sternType, &fbb)
    Vessel_Full.add(numBlades: numBlades, &fbb)
    Vessel_Full.add(numShafts: numShafts, &fbb)
    Vessel_Full.add(propType: propType, &fbb)
    Vessel_Full.add(hullNum: hullNum, &fbb)
    Vessel_Full.add(createdAt: createdAt, &fbb)
    Vessel_Full.add(createdBy: createdBy, &fbb)
    Vessel_Full.add(updatedAt: updatedAt, &fbb)
    Vessel_Full.add(updatedBy: updatedBy, &fbb)
    Vessel_Full.add(source: source, &fbb)
    Vessel_Full.add(sourceDl: sourceDl, &fbb)
    Vessel_Full.add(origin: origin, &fbb)
    Vessel_Full.add(dataMode: dataMode, &fbb)
    Vessel_Full.add(origNetwork: origNetwork, &fbb)
    return Vessel_Full.endVessel_Full(&fbb, start: __start)
  }
  ...
  public static func verify<T>(_ verifier: inout Verifier, at position: Int, of type: T.Type) throws where T: Verifiable {
    ...
    try _v.visit(field: VTOFFSET.dataMode.p, fieldName: "dataMode", required: false, type: Vessel_Full_dataMode_Enum.self)
