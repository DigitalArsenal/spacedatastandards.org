...
public struct LogisticsStocks_Full: FlatBufferObject, Verifiable {
  ...
  public static func finish(_ fbb: inout FlatBufferBuilder, end: Offset, prefix: Bool = false) { fbb.finish(offset: end, fileId: LogisticsStocks_Full.id, addPrefix: prefix) }
  ...
  public static func startLogisticsStocks_Full(_ fbb: inout FlatBufferBuilder) -> UOffset { fbb.startTable(with: 4) }
  ...
  public static func endLogisticsStocks_Full(_ fbb: inout FlatBufferBuilder, start: UOffset) -> Offset { let end = Offset(offset: fbb.endTable(at: start)); return end }
  public static func createLogisticsStocks_Full(
    ...
    let __start = LogisticsStocks_Full.startLogisticsStocks_Full(&fbb)
    LogisticsStocks_Full.add(sourceIcao: sourceIcao, &fbb)
    LogisticsStocks_Full.add(quantity: quantity, &fbb)
    LogisticsStocks_Full.add(stockCheckTime: stockCheckTime, &fbb)
    LogisticsStocks_Full.add(stockPoc: stockPoc, &fbb)
    return LogisticsStocks_Full.endLogisticsStocks_Full(&fbb, start: __start)
  }
