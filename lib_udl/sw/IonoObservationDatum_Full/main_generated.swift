...
public struct IonoObservationDatum_Full: FlatBufferObject, Verifiable {
  ...
  public static func finish(_ fbb: inout FlatBufferBuilder, end: Offset, prefix: Bool = false) { fbb.finish(offset: end, fileId: IonoObservationDatum_Full.id, addPrefix: prefix) }
  ...
  public static func startIonoObservationDatum_Full(_ fbb: inout FlatBufferBuilder) -> UOffset { fbb.startTable(with: 2) }
  ...
  public static func endIonoObservationDatum_Full(_ fbb: inout FlatBufferBuilder, start: UOffset) -> Offset { let end = Offset(offset: fbb.endTable(at: start)); return end }
  public static func createIonoObservationDatum_Full(
    _ fbb: inout FlatBufferBuilder,
    notesOffset notes: Offset = Offset(),
    dataVectorOffset data: Offset = Offset()
  ) -> Offset {
    let __start = IonoObservationDatum_Full.startIonoObservationDatum_Full(&fbb)
    IonoObservationDatum_Full.add(notes: notes, &fbb)
    IonoObservationDatum_Full.addVectorOf(data: data, &fbb)
    return IonoObservationDatum_Full.endIonoObservationDatum_Full(&fbb, start: __start)
  }
