...
public enum SARObservation_Full_dataMode_Enum: Int8, Enum, Verifiable {
  ...
  public static var max: SARObservation_Full_dataMode_Enum { return .exercise }
  public static var min: SARObservation_Full_dataMode_Enum { return .real }
}
...
public struct SARObservation_Full: FlatBufferObject, Verifiable {
  ...
  public static func finish(_ fbb: inout FlatBufferBuilder, end: Offset, prefix: Bool = false) { fbb.finish(offset: end, fileId: SARObservation_Full.id, addPrefix: prefix) }
  ...
  ///  Constraints: Minimum length = 1, Maximum length = 32
  public var dataMode: SARObservation_Full_dataMode_Enum { let o = _accessor.offset(VTOFFSET.dataMode.v); return o == 0 ? .real : SARObservation_Full_dataMode_Enum(rawValue: _accessor.readBuffer(of: Int8.self, at: o)) ?? .real }
  public static func startSARObservation_Full(_ fbb: inout FlatBufferBuilder) -> UOffset { fbb.startTable(with: 70) }
  ...
  public static func add(dataMode: SARObservation_Full_dataMode_Enum, _ fbb: inout FlatBufferBuilder) { fbb.add(element: dataMode.rawValue, def: 0, at: VTOFFSET.dataMode.p) }
  public static func endSARObservation_Full(_ fbb: inout FlatBufferBuilder, start: UOffset) -> Offset { let end = Offset(offset: fbb.endTable(at: start)); return end }
  public static func createSARObservation_Full(
    ...
    let __start = SARObservation_Full.startSARObservation_Full(&fbb)
    SARObservation_Full.add(id: id, &fbb)
    SARObservation_Full.add(classificationMarking: classificationMarking, &fbb)
    SARObservation_Full.add(idOnOrbit: idOnOrbit, &fbb)
    SARObservation_Full.add(satNo: satNo, &fbb)
    SARObservation_Full.add(origObjectId: origObjectId, &fbb)
    SARObservation_Full.add(idSensor: idSensor, &fbb)
    SARObservation_Full.add(origSensorId: origSensorId, &fbb)
    SARObservation_Full.add(externalId: externalId, &fbb)
    SARObservation_Full.add(collectionId: collectionId, &fbb)
    SARObservation_Full.add(detectionId: detectionId, &fbb)
    SARObservation_Full.add(collectionStart: collectionStart, &fbb)
    SARObservation_Full.add(collectionEnd: collectionEnd, &fbb)
    SARObservation_Full.add(centerTime: centerTime, &fbb)
    SARObservation_Full.add(detectionStart: detectionStart, &fbb)
    SARObservation_Full.add(detectionEnd: detectionEnd, &fbb)
    SARObservation_Full.add(dwellTime: dwellTime, &fbb)
    SARObservation_Full.add(orbitState: orbitState, &fbb)
    SARObservation_Full.add(sarMode: sarMode, &fbb)
    SARObservation_Full.add(operatingBand: operatingBand, &fbb)
    SARObservation_Full.add(operatingFreq: operatingFreq, &fbb)
    SARObservation_Full.add(snr: snr, &fbb)
    SARObservation_Full.add(txPolarization: txPolarization, &fbb)
    SARObservation_Full.add(rxPolarization: rxPolarization, &fbb)
    SARObservation_Full.add(grazeAngle: grazeAngle, &fbb)
    SARObservation_Full.add(incidenceAngle: incidenceAngle, &fbb)
    SARObservation_Full.add(squintAngle: squintAngle, &fbb)
    SARObservation_Full.add(pulseBandwidth: pulseBandwidth, &fbb)
    SARObservation_Full.add(pulseDuration: pulseDuration, &fbb)
    SARObservation_Full.add(continuousSpotAngle: continuousSpotAngle, &fbb)
    SARObservation_Full.add(slantRange: slantRange, &fbb)
    SARObservation_Full.add(nearRange: nearRange, &fbb)
    SARObservation_Full.add(farRange: farRange, &fbb)
    SARObservation_Full.add(swathLength: swathLength, &fbb)
    SARObservation_Full.add(area: area, &fbb)
    SARObservation_Full.add(atext: atext, &fbb)
    SARObservation_Full.add(agjson: agjson, &fbb)
    SARObservation_Full.add(atype: atype, &fbb)
    SARObservation_Full.add(andims: andims, &fbb)
    SARObservation_Full.add(asrid: asrid, &fbb)
    SARObservation_Full.add(spacingRange: spacingRange, &fbb)
    SARObservation_Full.add(spacingAzimuth: spacingAzimuth, &fbb)
    SARObservation_Full.add(looksAzimuth: looksAzimuth, &fbb)
    SARObservation_Full.add(looksRange: looksRange, &fbb)
    SARObservation_Full.add(resolutionRange: resolutionRange, &fbb)
    SARObservation_Full.add(resolutionAzimuth: resolutionAzimuth, &fbb)
    SARObservation_Full.add(obDirection: obDirection, &fbb)
    SARObservation_Full.add(coordSys: coordSys, &fbb)
    SARObservation_Full.add(targetposx: targetposx, &fbb)
    SARObservation_Full.add(targetposy: targetposy, &fbb)
    SARObservation_Full.add(targetposz: targetposz, &fbb)
    SARObservation_Full.add(senalt: senalt, &fbb)
    SARObservation_Full.add(senvelx: senvelx, &fbb)
    SARObservation_Full.add(senvely: senvely, &fbb)
    SARObservation_Full.add(senvelz: senvelz, &fbb)
    SARObservation_Full.add(senlatStart: senlatStart, &fbb)
    SARObservation_Full.add(senlonStart: senlonStart, &fbb)
    SARObservation_Full.add(senlatEnd: senlatEnd, &fbb)
    SARObservation_Full.add(senlonEnd: senlonEnd, &fbb)
    SARObservation_Full.add(transactionId: transactionId, &fbb)
    SARObservation_Full.addVectorOf(tags: tags, &fbb)
    SARObservation_Full.addVectorOf(srcTyps: srcTyps, &fbb)
    SARObservation_Full.addVectorOf(srcIds: srcIds, &fbb)
    SARObservation_Full.add(onOrbit: onOrbit, &fbb)
    SARObservation_Full.add(createdAt: createdAt, &fbb)
    SARObservation_Full.add(createdBy: createdBy, &fbb)
    SARObservation_Full.add(source: source, &fbb)
    SARObservation_Full.add(origin: origin, &fbb)
    SARObservation_Full.add(origNetwork: origNetwork, &fbb)
    SARObservation_Full.add(sourceDl: sourceDl, &fbb)
    SARObservation_Full.add(dataMode: dataMode, &fbb)
    return SARObservation_Full.endSARObservation_Full(&fbb, start: __start)
  }
  ...
  public static func verify<T>(_ verifier: inout Verifier, at position: Int, of type: T.Type) throws where T: Verifiable {
    ...
    try _v.visit(field: VTOFFSET.dataMode.p, fieldName: "dataMode", required: false, type: SARObservation_Full_dataMode_Enum.self)
    _v.finish()
  }
}
