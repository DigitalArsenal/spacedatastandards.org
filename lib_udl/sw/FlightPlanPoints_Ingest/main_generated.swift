...
public struct FlightPlanPoints_Ingest: FlatBufferObject, Verifiable {
  ...
  public static func finish(_ fbb: inout FlatBufferBuilder, end: Offset, prefix: Bool = false) { fbb.finish(offset: end, fileId: FlightPlanPoints_Ingest.id, addPrefix: prefix) }
  ...
  public static func startFlightPlanPoints_Ingest(_ fbb: inout FlatBufferBuilder) -> UOffset { fbb.startTable(with: 5) }
  ...
  public static func endFlightPlanPoints_Ingest(_ fbb: inout FlatBufferBuilder, start: UOffset) -> Offset { let end = Offset(offset: fbb.endTable(at: start)); return end }
  public static func createFlightPlanPoints_Ingest(
    ...
    let __start = FlightPlanPoints_Ingest.startFlightPlanPoints_Ingest(&fbb)
    FlightPlanPoints_Ingest.add(pointName: pointName, &fbb)
    FlightPlanPoints_Ingest.add(fppLat: fppLat, &fbb)
    FlightPlanPoints_Ingest.add(fppLon: fppLon, &fbb)
    FlightPlanPoints_Ingest.add(fppEta: fppEta, &fbb)
    FlightPlanPoints_Ingest.add(fppReqFuel: fppReqFuel, &fbb)
    return FlightPlanPoints_Ingest.endFlightPlanPoints_Ingest(&fbb, start: __start)
  }
