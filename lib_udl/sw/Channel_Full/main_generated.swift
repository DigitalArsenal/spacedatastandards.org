...
public enum Channel_Full_dataMode_Enum: Int8, Enum, Verifiable {
  ...
  public static var max: Channel_Full_dataMode_Enum { return .exercise }
  public static var min: Channel_Full_dataMode_Enum { return .real }
}


///  /// Channel information on a particular transponder.
public struct Channel_Full: FlatBufferObject, Verifiable {
  ...
  public static func finish(_ fbb: inout FlatBufferBuilder, end: Offset, prefix: Bool = false) { fbb.finish(offset: end, fileId: Channel_Full.id, addPrefix: prefix) }
  ...
  ///  Constraints: Minimum length = 1, Maximum length = 32
  public var dataMode: Channel_Full_dataMode_Enum { let o = _accessor.offset(VTOFFSET.dataMode.v); return o == 0 ? .real : Channel_Full_dataMode_Enum(rawValue: _accessor.readBuffer(of: Int8.self, at: o)) ?? .real }
  ...
  public static func startChannel_Full(_ fbb: inout FlatBufferBuilder) -> UOffset { fbb.startTable(with: 24) }
  ...
  public static func add(dataMode: Channel_Full_dataMode_Enum, _ fbb: inout FlatBufferBuilder) { fbb.add(element: dataMode.rawValue, def: 0, at: VTOFFSET.dataMode.p) }
  ...
  public static func endChannel_Full(_ fbb: inout FlatBufferBuilder, start: UOffset) -> Offset { let end = Offset(offset: fbb.endTable(at: start)); return end }
  public static func createChannel_Full(
    ...
    let __start = Channel_Full.startChannel_Full(&fbb)
    Channel_Full.add(id: id, &fbb)
    Channel_Full.add(idTransponder: idTransponder, &fbb)
    Channel_Full.add(name: name, &fbb)
    Channel_Full.add(classificationMarking: classificationMarking, &fbb)
    Channel_Full.add(type: type, &fbb)
    Channel_Full.add(idBeam: idBeam, &fbb)
    Channel_Full.add(beamName: beamName, &fbb)
    Channel_Full.add(idRfband: idRfband, &fbb)
    Channel_Full.add(encryption: encryption, &fbb)
    Channel_Full.add(pkg: pkg, &fbb)
    Channel_Full.add(res: res, &fbb)
    Channel_Full.add(compression: compression, &fbb)
    Channel_Full.add(vpid: vpid, &fbb)
    Channel_Full.add(apid: apid, &fbb)
    Channel_Full.add(sid: sid, &fbb)
    Channel_Full.add(owner: owner, &fbb)
    Channel_Full.add(source: source, &fbb)
    Channel_Full.add(origin: origin, &fbb)
    Channel_Full.add(origNetwork: origNetwork, &fbb)
    Channel_Full.add(dataMode: dataMode, &fbb)
    Channel_Full.add(createdAt: createdAt, &fbb)
    Channel_Full.add(createdBy: createdBy, &fbb)
    Channel_Full.add(updatedAt: updatedAt, &fbb)
    Channel_Full.add(updatedBy: updatedBy, &fbb)
    return Channel_Full.endChannel_Full(&fbb, start: __start)
  }
  ...
  public static func verify<T>(_ verifier: inout Verifier, at position: Int, of type: T.Type) throws where T: Verifiable {
    ...
    try _v.visit(field: VTOFFSET.dataMode.p, fieldName: "dataMode", required: false, type: Channel_Full_dataMode_Enum.self)
