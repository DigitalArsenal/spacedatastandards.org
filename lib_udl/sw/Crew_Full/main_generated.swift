...
public enum Crew_Full_dataMode_Enum: Int8, Enum, Verifiable {
  ...
  public static var max: Crew_Full_dataMode_Enum { return .exercise }
  public static var min: Crew_Full_dataMode_Enum { return .real }
}


///  /// Crew Services.
public struct Crew_Full: FlatBufferObject, Verifiable {
  ...
  public static func finish(_ fbb: inout FlatBufferBuilder, end: Offset, prefix: Bool = false) { fbb.finish(offset: end, fileId: Crew_Full.id, addPrefix: prefix) }
  ...
  public var dataMode: Crew_Full_dataMode_Enum { let o = _accessor.offset(VTOFFSET.dataMode.v); return o == 0 ? .real : Crew_Full_dataMode_Enum(rawValue: _accessor.readBuffer(of: Int8.self, at: o)) ?? .real }
  public static func startCrew_Full(_ fbb: inout FlatBufferBuilder) -> UOffset { fbb.startTable(with: 23) }
  ...
  public static func add(dataMode: Crew_Full_dataMode_Enum, _ fbb: inout FlatBufferBuilder) { fbb.add(element: dataMode.rawValue, def: 0, at: VTOFFSET.dataMode.p) }
  public static func endCrew_Full(_ fbb: inout FlatBufferBuilder, start: UOffset) -> Offset { let end = Offset(offset: fbb.endTable(at: start)); return end }
  public static func createCrew_Full(
    ...
    let __start = Crew_Full.startCrew_Full(&fbb)
    Crew_Full.add(id: id, &fbb)
    Crew_Full.add(classificationMarking: classificationMarking, &fbb)
    Crew_Full.add(origCrewId: origCrewId, &fbb)
    Crew_Full.add(crewName: crewName, &fbb)
    Crew_Full.add(status: status, &fbb)
    Crew_Full.add(idSortie: idSortie, &fbb)
    Crew_Full.add(legalAlertTime: legalAlertTime, &fbb)
    Crew_Full.add(legalBravoTime: legalBravoTime, &fbb)
    Crew_Full.add(alertedTime: alertedTime, &fbb)
    Crew_Full.add(returnTime: returnTime, &fbb)
    Crew_Full.add(fdpEligType: fdpEligType, &fbb)
    Crew_Full.add(fdpType: fdpType, &fbb)
    Crew_Full.add(crewType: crewType, &fbb)
    Crew_Full.add(armsCrewUnit: armsCrewUnit, &fbb)
    Crew_Full.addVectorOf(crewMembers: crewMembers, &fbb)
    Crew_Full.add(createdAt: createdAt, &fbb)
    Crew_Full.add(createdBy: createdBy, &fbb)
    Crew_Full.add(updatedAt: updatedAt, &fbb)
    Crew_Full.add(updatedBy: updatedBy, &fbb)
    Crew_Full.add(source: source, &fbb)
    Crew_Full.add(origin: origin, &fbb)
    Crew_Full.add(origNetwork: origNetwork, &fbb)
    Crew_Full.add(dataMode: dataMode, &fbb)
    return Crew_Full.endCrew_Full(&fbb, start: __start)
  }
  ...
  public static func verify<T>(_ verifier: inout Verifier, at position: Int, of type: T.Type) throws where T: Verifiable {
    ...
    try _v.visit(field: VTOFFSET.dataMode.p, fieldName: "dataMode", required: false, type: Crew_Full_dataMode_Enum.self)
    _v.finish()
  }
}
