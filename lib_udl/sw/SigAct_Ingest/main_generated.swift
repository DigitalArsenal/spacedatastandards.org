...
public enum SigAct_Ingest_dataMode_Enum: Int8, Enum, Verifiable {
  ...
  public static var max: SigAct_Ingest_dataMode_Enum { return .exercise }
  public static var min: SigAct_Ingest_dataMode_Enum { return .real }
}
...
public struct SigAct_Ingest: FlatBufferObject, Verifiable {
  ...
  public static func finish(_ fbb: inout FlatBufferBuilder, end: Offset, prefix: Bool = false) { fbb.finish(offset: end, fileId: SigAct_Ingest.id, addPrefix: prefix) }
  ...
  private enum VTOFFSET: VOffset {
    ...
    case friendlyDet = 70
    case friendlyKia = 72
    case friendlyWound = 74
    case hostNatAbd = 76
    case hostNatDet = 78
    case hostNatKia = 80
    case hostNatWound = 82
    case sideAabd = 84
    case sideAdet = 86
    case sideAkia = 88
    case sideAwound = 90
    ...
    case sideBwound = 98
    case target = 100
    case eventDescription = 102
    case eventType = 104
    case typeOfAttack = 106
    case complexAttack = 108
    case idNumber = 110
    case confidence = 112
    case accuracy = 114
    case clarity = 116
    case cameoCode = 118
    case cameoBaseCode = 120
    case cameoRootCode = 122
    case goldstein = 124
    case documentSource = 126
    case documentFilename = 128
    case hasAttachment = 130
    case filesize = 132
    case checksumValue = 134
    case repUnit = 136
    case repUnitType = 138
    case repUnitActivity = 140
    case numMentions = 142
    case numSources = 144
    case numArticles = 146
    case avgTone = 148
    case sourceUrl = 150
    case relatedDocs = 152
    case summary = 154
    case notes = 156
    case createdAt = 158
    case createdBy = 160
  ...
  public var dataMode: SigAct_Ingest_dataMode_Enum { let o = _accessor.offset(VTOFFSET.dataMode.v); return o == 0 ? .real : SigAct_Ingest_dataMode_Enum(rawValue: _accessor.readBuffer(of: Int8.self, at: o)) ?? .real }
  ///  The originating source network on which this record was created, auto-populated by the system.
  ///  Example: /// Example: ORIG
  ///  Constraints: Minimum length = 1, Maximum length = 32
  public var origNetwork: String? { let o = _accessor.offset(VTOFFSET.origNetwork.v); return o == 0 ? nil : _accessor.string(at: o) }
  public var origNetworkSegmentArray: [UInt8]? { return _accessor.getVector(at: VTOFFSET.origNetwork.v) }
  public static func startSigAct_Ingest(_ fbb: inout FlatBufferBuilder) -> UOffset { fbb.startTable(with: 83) }
  ...
  public static func add(dataMode: SigAct_Ingest_dataMode_Enum, _ fbb: inout FlatBufferBuilder) { fbb.add(element: dataMode.rawValue, def: 0, at: VTOFFSET.dataMode.p) }
  public static func add(origNetwork: Offset, _ fbb: inout FlatBufferBuilder) { fbb.add(offset: origNetwork, at: VTOFFSET.origNetwork.p) }
  public static func endSigAct_Ingest(_ fbb: inout FlatBufferBuilder, start: UOffset) -> Offset { let end = Offset(offset: fbb.endTable(at: start)); return end }
  public static func createSigAct_Ingest(
    ...
    let __start = SigAct_Ingest.startSigAct_Ingest(&fbb)
    SigAct_Ingest.add(id: id, &fbb)
    SigAct_Ingest.add(classificationMarking: classificationMarking, &fbb)
    SigAct_Ingest.add(sourceLanguage: sourceLanguage, &fbb)
    SigAct_Ingest.add(countryCode: countryCode, &fbb)
    SigAct_Ingest.add(city: city, &fbb)
    SigAct_Ingest.add(district: district, &fbb)
    SigAct_Ingest.add(province: province, &fbb)
    SigAct_Ingest.add(lat: lat, &fbb)
    SigAct_Ingest.add(lon: lon, &fbb)
    SigAct_Ingest.add(milgrid: milgrid, &fbb)
    SigAct_Ingest.add(theater: theater, &fbb)
    SigAct_Ingest.add(area: area, &fbb)
    SigAct_Ingest.add(atext: atext, &fbb)
    SigAct_Ingest.add(agjson: agjson, &fbb)
    SigAct_Ingest.add(atype: atype, &fbb)
    SigAct_Ingest.add(andims: andims, &fbb)
    SigAct_Ingest.add(asrid: asrid, &fbb)
    SigAct_Ingest.add(eventStart: eventStart, &fbb)
    SigAct_Ingest.add(eventEnd: eventEnd, &fbb)
    SigAct_Ingest.add(reportDate: reportDate, &fbb)
    SigAct_Ingest.addVectorOf(actors: actors, &fbb)
    SigAct_Ingest.add(civAbd: civAbd, &fbb)
    SigAct_Ingest.add(civDet: civDet, &fbb)
    SigAct_Ingest.add(civKia: civKia, &fbb)
    SigAct_Ingest.add(civWound: civWound, &fbb)
    SigAct_Ingest.add(coalAbd: coalAbd, &fbb)
    SigAct_Ingest.add(coalDet: coalDet, &fbb)
    SigAct_Ingest.add(coalKia: coalKia, &fbb)
    SigAct_Ingest.add(coalWound: coalWound, &fbb)
    SigAct_Ingest.add(enemyAbd: enemyAbd, &fbb)
    SigAct_Ingest.add(enemyDet: enemyDet, &fbb)
    SigAct_Ingest.add(enemyKia: enemyKia, &fbb)
    SigAct_Ingest.add(friendlyAbd: friendlyAbd, &fbb)
    SigAct_Ingest.add(friendlyDet: friendlyDet, &fbb)
    SigAct_Ingest.add(friendlyKia: friendlyKia, &fbb)
    SigAct_Ingest.add(friendlyWound: friendlyWound, &fbb)
    SigAct_Ingest.add(hostNatAbd: hostNatAbd, &fbb)
    SigAct_Ingest.add(hostNatDet: hostNatDet, &fbb)
    SigAct_Ingest.add(hostNatKia: hostNatKia, &fbb)
    SigAct_Ingest.add(hostNatWound: hostNatWound, &fbb)
    SigAct_Ingest.add(sideAabd: sideAabd, &fbb)
    SigAct_Ingest.add(sideAdet: sideAdet, &fbb)
    SigAct_Ingest.add(sideAkia: sideAkia, &fbb)
    SigAct_Ingest.add(sideAwound: sideAwound, &fbb)
    SigAct_Ingest.add(sideBabd: sideBabd, &fbb)
    SigAct_Ingest.add(sideBdet: sideBdet, &fbb)
    SigAct_Ingest.add(sideBkia: sideBkia, &fbb)
    SigAct_Ingest.add(sideBwound: sideBwound, &fbb)
    SigAct_Ingest.add(target: target, &fbb)
    SigAct_Ingest.add(eventDescription: eventDescription, &fbb)
    SigAct_Ingest.add(eventType: eventType, &fbb)
    SigAct_Ingest.add(typeOfAttack: typeOfAttack, &fbb)
    SigAct_Ingest.add(complexAttack: complexAttack, &fbb)
    SigAct_Ingest.add(idNumber: idNumber, &fbb)
    SigAct_Ingest.add(confidence: confidence, &fbb)
    SigAct_Ingest.add(accuracy: accuracy, &fbb)
    SigAct_Ingest.add(clarity: clarity, &fbb)
    SigAct_Ingest.add(cameoCode: cameoCode, &fbb)
    SigAct_Ingest.add(cameoBaseCode: cameoBaseCode, &fbb)
    SigAct_Ingest.add(cameoRootCode: cameoRootCode, &fbb)
    SigAct_Ingest.add(goldstein: goldstein, &fbb)
    SigAct_Ingest.add(documentSource: documentSource, &fbb)
    SigAct_Ingest.add(documentFilename: documentFilename, &fbb)
    SigAct_Ingest.add(hasAttachment: hasAttachment, &fbb)
    SigAct_Ingest.add(filesize: filesize, &fbb)
    SigAct_Ingest.add(checksumValue: checksumValue, &fbb)
    SigAct_Ingest.add(repUnit: repUnit, &fbb)
    SigAct_Ingest.add(repUnitType: repUnitType, &fbb)
    SigAct_Ingest.add(repUnitActivity: repUnitActivity, &fbb)
    SigAct_Ingest.add(numMentions: numMentions, &fbb)
    SigAct_Ingest.add(numSources: numSources, &fbb)
    SigAct_Ingest.add(numArticles: numArticles, &fbb)
    SigAct_Ingest.add(avgTone: avgTone, &fbb)
    SigAct_Ingest.add(sourceUrl: sourceUrl, &fbb)
    SigAct_Ingest.addVectorOf(relatedDocs: relatedDocs, &fbb)
    SigAct_Ingest.add(summary: summary, &fbb)
    SigAct_Ingest.add(notes: notes, &fbb)
    SigAct_Ingest.add(createdAt: createdAt, &fbb)
    SigAct_Ingest.add(createdBy: createdBy, &fbb)
    SigAct_Ingest.add(source: source, &fbb)
    SigAct_Ingest.add(origin: origin, &fbb)
    SigAct_Ingest.add(dataMode: dataMode, &fbb)
    SigAct_Ingest.add(origNetwork: origNetwork, &fbb)
    return SigAct_Ingest.endSigAct_Ingest(&fbb, start: __start)
  }
  ...
  public static func verify<T>(_ verifier: inout Verifier, at position: Int, of type: T.Type) throws where T: Verifiable {
    ...
    try _v.visit(field: VTOFFSET.dataMode.p, fieldName: "dataMode", required: false, type: SigAct_Ingest_dataMode_Enum.self)
