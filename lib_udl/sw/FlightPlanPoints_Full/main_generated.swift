...
public struct FlightPlanPoints_Full: FlatBufferObject, Verifiable {
  ...
  public static func finish(_ fbb: inout FlatBufferBuilder, end: Offset, prefix: Bool = false) { fbb.finish(offset: end, fileId: FlightPlanPoints_Full.id, addPrefix: prefix) }
  ...
  public static func startFlightPlanPoints_Full(_ fbb: inout FlatBufferBuilder) -> UOffset { fbb.startTable(with: 5) }
  ...
  public static func endFlightPlanPoints_Full(_ fbb: inout FlatBufferBuilder, start: UOffset) -> Offset { let end = Offset(offset: fbb.endTable(at: start)); return end }
  public static func createFlightPlanPoints_Full(
    ...
    let __start = FlightPlanPoints_Full.startFlightPlanPoints_Full(&fbb)
    FlightPlanPoints_Full.add(pointName: pointName, &fbb)
    FlightPlanPoints_Full.add(fppLat: fppLat, &fbb)
    FlightPlanPoints_Full.add(fppLon: fppLon, &fbb)
    FlightPlanPoints_Full.add(fppEta: fppEta, &fbb)
    FlightPlanPoints_Full.add(fppReqFuel: fppReqFuel, &fbb)
    return FlightPlanPoints_Full.endFlightPlanPoints_Full(&fbb, start: __start)
  }
