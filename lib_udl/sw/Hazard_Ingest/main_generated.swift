...
public enum Hazard_Ingest_dataMode_Enum: Int8, Enum, Verifiable {
  ...
  public static var max: Hazard_Ingest_dataMode_Enum { return .exercise }
  public static var min: Hazard_Ingest_dataMode_Enum { return .real }
}
...
public struct Hazard_Ingest: FlatBufferObject, Verifiable {
  ...
  public static func finish(_ fbb: inout FlatBufferBuilder, end: Offset, prefix: Bool = false) { fbb.finish(offset: end, fileId: Hazard_Ingest.id, addPrefix: prefix) }
  ...
  ///  Constraints: Minimum length = 1, Maximum length = 32
  public var dataMode: Hazard_Ingest_dataMode_Enum { let o = _accessor.offset(VTOFFSET.dataMode.v); return o == 0 ? .real : Hazard_Ingest_dataMode_Enum(rawValue: _accessor.readBuffer(of: Int8.self, at: o)) ?? .real }
  public static func startHazard_Ingest(_ fbb: inout FlatBufferBuilder) -> UOffset { fbb.startTable(with: 40) }
  ...
  public static func add(dataMode: Hazard_Ingest_dataMode_Enum, _ fbb: inout FlatBufferBuilder) { fbb.add(element: dataMode.rawValue, def: 0, at: VTOFFSET.dataMode.p) }
  public static func endHazard_Ingest(_ fbb: inout FlatBufferBuilder, start: UOffset) -> Offset { let end = Offset(offset: fbb.endTable(at: start)); return end }
  public static func createHazard_Ingest(
    ...
    let __start = Hazard_Ingest.startHazard_Ingest(&fbb)
    Hazard_Ingest.add(id: id, &fbb)
    Hazard_Ingest.add(classificationMarking: classificationMarking, &fbb)
    Hazard_Ingest.add(detectType: detectType, &fbb)
    Hazard_Ingest.add(detectTime: detectTime, &fbb)
    Hazard_Ingest.add(harmful: harmful, &fbb)
    Hazard_Ingest.addVectorOf(alarms: alarms, &fbb)
    Hazard_Ingest.addVectorOf(alarmValues: alarmValues, &fbb)
    Hazard_Ingest.addVectorOf(readings: readings, &fbb)
    Hazard_Ingest.addVectorOf(readingUnits: readingUnits, &fbb)
    Hazard_Ingest.addVectorOf(readingValues: readingValues, &fbb)
    Hazard_Ingest.add(matType: matType, &fbb)
    Hazard_Ingest.add(matClass: matClass, &fbb)
    Hazard_Ingest.add(matName: matName, &fbb)
    Hazard_Ingest.add(matCat: matCat, &fbb)
    Hazard_Ingest.add(casRn: casRn, &fbb)
    Hazard_Ingest.add(ppm: ppm, &fbb)
    Hazard_Ingest.add(density: density, &fbb)
    Hazard_Ingest.add(radCtrn: radCtrn, &fbb)
    Hazard_Ingest.add(gBar: gBar, &fbb)
    Hazard_Ingest.add(hBar: hBar, &fbb)
    Hazard_Ingest.add(z: z, &fbb)
    Hazard_Ingest.add(a: a, &fbb)
    Hazard_Ingest.add(bottleId: bottleId, &fbb)
    Hazard_Ingest.add(channel: channel, &fbb)
    Hazard_Ingest.add(duration: duration, &fbb)
    Hazard_Ingest.add(activity: activity, &fbb)
    Hazard_Ingest.add(dep: dep, &fbb)
    Hazard_Ingest.add(depCtrn: depCtrn, &fbb)
    Hazard_Ingest.add(massFrac: massFrac, &fbb)
    Hazard_Ingest.add(dose: dose, &fbb)
    Hazard_Ingest.add(ctrnTime: ctrnTime, &fbb)
    Hazard_Ingest.add(doseRate: doseRate, &fbb)
    Hazard_Ingest.add(idPoi: idPoi, &fbb)
    Hazard_Ingest.add(idTrack: idTrack, &fbb)
    Hazard_Ingest.add(createdAt: createdAt, &fbb)
    Hazard_Ingest.add(createdBy: createdBy, &fbb)
    Hazard_Ingest.add(source: source, &fbb)
    Hazard_Ingest.add(origin: origin, &fbb)
    Hazard_Ingest.add(origNetwork: origNetwork, &fbb)
    Hazard_Ingest.add(dataMode: dataMode, &fbb)
    return Hazard_Ingest.endHazard_Ingest(&fbb, start: __start)
  }
  ...
  public static func verify<T>(_ verifier: inout Verifier, at position: Int, of type: T.Type) throws where T: Verifiable {
    ...
    try _v.visit(field: VTOFFSET.dataMode.p, fieldName: "dataMode", required: false, type: Hazard_Ingest_dataMode_Enum.self)
    _v.finish()
  }
}
