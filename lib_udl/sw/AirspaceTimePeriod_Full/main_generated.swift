...
public struct AirspaceTimePeriod_Full: FlatBufferObject, Verifiable {
  ...
  public static func finish(_ fbb: inout FlatBufferBuilder, end: Offset, prefix: Bool = false) { fbb.finish(offset: end, fileId: AirspaceTimePeriod_Full.id, addPrefix: prefix) }
  ...
  public static func startAirspaceTimePeriod_Full(_ fbb: inout FlatBufferBuilder) -> UOffset { fbb.startTable(with: 5) }
  ...
  public static func endAirspaceTimePeriod_Full(_ fbb: inout FlatBufferBuilder, start: UOffset) -> Offset { let end = Offset(offset: fbb.endTable(at: start)); return end }
  public static func createAirspaceTimePeriod_Full(
    ...
    let __start = AirspaceTimePeriod_Full.startAirspaceTimePeriod_Full(&fbb)
    AirspaceTimePeriod_Full.add(timeMode: timeMode, &fbb)
    AirspaceTimePeriod_Full.add(timeStart: timeStart, &fbb)
    AirspaceTimePeriod_Full.add(timeEnd: timeEnd, &fbb)
    AirspaceTimePeriod_Full.addVectorOf(intFreq: intFreq, &fbb)
    AirspaceTimePeriod_Full.addVectorOf(intDur: intDur, &fbb)
    return AirspaceTimePeriod_Full.endAirspaceTimePeriod_Full(&fbb, start: __start)
  }
