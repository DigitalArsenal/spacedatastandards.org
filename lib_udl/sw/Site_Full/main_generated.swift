...
public enum Site_Full_dataMode_Enum: Int8, Enum, Verifiable {
  ...
  public static var max: Site_Full_dataMode_Enum { return .exercise }
  public static var min: Site_Full_dataMode_Enum { return .real }
}


///  /// Properties and characteristics of a site entity, such as an airbase, airfield, naval station, etc.
public struct Site_Full: FlatBufferObject, Verifiable {
  ...
  public static func finish(_ fbb: inout FlatBufferBuilder, end: Offset, prefix: Bool = false) { fbb.finish(offset: end, fileId: Site_Full.id, addPrefix: prefix) }
  ...
  private enum VTOFFSET: VOffset {
    ...
    case entity = 6
    case idEntity = 8
    case idParentSite = 10
    case classificationMarking = 12
    case origSiteId = 14
    case origLzId = 16
    case pin = 18
    case type = 20
    ...
    case polSubdiv = 56
    case geoArea = 58
    case geoidalMslSep = 60
    case allegiance = 62
    case altAllegiance = 64
    case classRating = 66
    case fpa = 68
    case beNumber = 70
    case runways = 72
    case maxRunwayLength = 74
    case condition = 76
    case conditionAvail = 78
    case operStatus = 80
    case grade = 82
    case activity = 84
    case functPrimary = 86
    case msnPrimary = 88
    case msnPrimarySpec = 90
    case nucCap = 92
    case ident = 94
    case elevMsl = 96
    case elevMslConfLvl = 98
    case elevMslDerivAcc = 100
    case vegHt = 102
    case vegType = 104
    case popArea = 106
    case popAreaProx = 108
    case notes = 110
    case resProd = 112
    case eval = 114
    case reviewDate = 116
    case recStatus = 118
    case symCode = 120
    case referenceDoc = 122
    case siteRemarks = 124
    case createdAt = 126
    case createdBy = 128
    case updatedAt = 130
    case updatedBy = 132
  ...
  ///  Example: /// Example: No example provided.
  ///  Constraints: No constraints specified.
  public var hasSiteRemarks: Bool { let o = _accessor.offset(VTOFFSET.siteRemarks.v); return o == 0 ? false : true }
  public var siteRemarksCount: Int32 { let o = _accessor.offset(VTOFFSET.siteRemarks.v); return o == 0 ? 0 : _accessor.vector(count: o) }
  public func siteRemarks(at index: Int32) -> String? { let o = _accessor.offset(VTOFFSET.siteRemarks.v); return o == 0 ? nil : _accessor.directString(at: _accessor.vector(at: o) + index * 4) }
  ...
  ///  Constraints: Minimum length = 1, Maximum length = 32
  public var dataMode: Site_Full_dataMode_Enum { let o = _accessor.offset(VTOFFSET.dataMode.v); return o == 0 ? .real : Site_Full_dataMode_Enum(rawValue: _accessor.readBuffer(of: Int8.self, at: o)) ?? .real }
  public static func startSite_Full(_ fbb: inout FlatBufferBuilder) -> UOffset { fbb.startTable(with: 69) }
  ...
  public static func addVectorOf(siteRemarks: Offset, _ fbb: inout FlatBufferBuilder) { fbb.add(offset: siteRemarks, at: VTOFFSET.siteRemarks.p) }
  ...
  public static func add(dataMode: Site_Full_dataMode_Enum, _ fbb: inout FlatBufferBuilder) { fbb.add(element: dataMode.rawValue, def: 0, at: VTOFFSET.dataMode.p) }
  public static func endSite_Full(_ fbb: inout FlatBufferBuilder, start: UOffset) -> Offset { let end = Offset(offset: fbb.endTable(at: start)); return end }
  public static func createSite_Full(
    ...
    siteRemarksVectorOffset siteRemarks: Offset = Offset(),
    ...
    let __start = Site_Full.startSite_Full(&fbb)
    Site_Full.add(id: id, &fbb)
    Site_Full.add(entity: entity, &fbb)
    Site_Full.add(idEntity: idEntity, &fbb)
    Site_Full.add(idParentSite: idParentSite, &fbb)
    Site_Full.add(classificationMarking: classificationMarking, &fbb)
    Site_Full.add(origSiteId: origSiteId, &fbb)
    Site_Full.add(origLzId: origLzId, &fbb)
    Site_Full.add(pin: pin, &fbb)
    Site_Full.add(type: type, &fbb)
    Site_Full.add(usage: usage, &fbb)
    Site_Full.add(lzUsage: lzUsage, &fbb)
    Site_Full.add(name: name, &fbb)
    Site_Full.add(coordDatum: coordDatum, &fbb)
    Site_Full.add(coord: coord, &fbb)
    Site_Full.add(coordDerivAcc: coordDerivAcc, &fbb)
    Site_Full.add(osuffix: osuffix, &fbb)
    Site_Full.add(iata: iata, &fbb)
    Site_Full.add(icao: icao, &fbb)
    Site_Full.add(faa: faa, &fbb)
    Site_Full.add(airDefArea: airDefArea, &fbb)
    Site_Full.add(utm: utm, &fbb)
    Site_Full.add(milGridSys: milGridSys, &fbb)
    Site_Full.add(milGrid: milGrid, &fbb)
    Site_Full.add(catCode: catCode, &fbb)
    Site_Full.add(catText: catText, &fbb)
    Site_Full.add(wac: wac, &fbb)
    Site_Full.add(polSubdiv: polSubdiv, &fbb)
    Site_Full.add(geoArea: geoArea, &fbb)
    Site_Full.add(geoidalMslSep: geoidalMslSep, &fbb)
    Site_Full.add(allegiance: allegiance, &fbb)
    Site_Full.add(altAllegiance: altAllegiance, &fbb)
    Site_Full.add(classRating: classRating, &fbb)
    Site_Full.add(fpa: fpa, &fbb)
    Site_Full.add(beNumber: beNumber, &fbb)
    Site_Full.add(runways: runways, &fbb)
    Site_Full.add(maxRunwayLength: maxRunwayLength, &fbb)
    Site_Full.add(condition: condition, &fbb)
    Site_Full.add(conditionAvail: conditionAvail, &fbb)
    Site_Full.add(operStatus: operStatus, &fbb)
    Site_Full.add(grade: grade, &fbb)
    Site_Full.add(activity: activity, &fbb)
    Site_Full.add(functPrimary: functPrimary, &fbb)
    Site_Full.add(msnPrimary: msnPrimary, &fbb)
    Site_Full.add(msnPrimarySpec: msnPrimarySpec, &fbb)
    Site_Full.add(nucCap: nucCap, &fbb)
    Site_Full.add(ident: ident, &fbb)
    Site_Full.add(elevMsl: elevMsl, &fbb)
    Site_Full.add(elevMslConfLvl: elevMslConfLvl, &fbb)
    Site_Full.add(elevMslDerivAcc: elevMslDerivAcc, &fbb)
    Site_Full.add(vegHt: vegHt, &fbb)
    Site_Full.add(vegType: vegType, &fbb)
    Site_Full.add(popArea: popArea, &fbb)
    Site_Full.add(popAreaProx: popAreaProx, &fbb)
    Site_Full.add(notes: notes, &fbb)
    Site_Full.add(resProd: resProd, &fbb)
    Site_Full.add(eval: eval, &fbb)
    Site_Full.add(reviewDate: reviewDate, &fbb)
    Site_Full.add(recStatus: recStatus, &fbb)
    Site_Full.add(symCode: symCode, &fbb)
    Site_Full.add(referenceDoc: referenceDoc, &fbb)
    Site_Full.addVectorOf(siteRemarks: siteRemarks, &fbb)
    Site_Full.add(createdAt: createdAt, &fbb)
    Site_Full.add(createdBy: createdBy, &fbb)
    Site_Full.add(updatedAt: updatedAt, &fbb)
    Site_Full.add(updatedBy: updatedBy, &fbb)
    Site_Full.add(source: source, &fbb)
    Site_Full.add(origin: origin, &fbb)
    Site_Full.add(origNetwork: origNetwork, &fbb)
    Site_Full.add(dataMode: dataMode, &fbb)
    return Site_Full.endSite_Full(&fbb, start: __start)
  }
  ...
  public static func verify<T>(_ verifier: inout Verifier, at position: Int, of type: T.Type) throws where T: Verifiable {
    ...
    try _v.visit(field: VTOFFSET.siteRemarks.p, fieldName: "siteRemarks", required: false, type: ForwardOffset<Vector<ForwardOffset<String>, String>>.self)
    ...
    try _v.visit(field: VTOFFSET.dataMode.p, fieldName: "dataMode", required: false, type: Site_Full_dataMode_Enum.self)
    _v.finish()
  }
}
