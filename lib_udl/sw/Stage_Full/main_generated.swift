...
public enum Stage_Full_dataMode_Enum: Int8, Enum, Verifiable {
  ...
  public static var max: Stage_Full_dataMode_Enum { return .exercise }
  public static var min: Stage_Full_dataMode_Enum { return .real }
}
...
public struct Stage_Full: FlatBufferObject, Verifiable {
  ...
  public static func finish(_ fbb: inout FlatBufferBuilder, end: Offset, prefix: Bool = false) { fbb.finish(offset: end, fileId: Stage_Full.id, addPrefix: prefix) }
  ...
  ///  Constraints: Minimum length = 1, Maximum length = 32
  public var dataMode: Stage_Full_dataMode_Enum { let o = _accessor.offset(VTOFFSET.dataMode.v); return o == 0 ? .real : Stage_Full_dataMode_Enum(rawValue: _accessor.readBuffer(of: Int8.self, at: o)) ?? .real }
  ...
  public static func startStage_Full(_ fbb: inout FlatBufferBuilder) -> UOffset { fbb.startTable(with: 43) }
  ...
  public static func add(dataMode: Stage_Full_dataMode_Enum, _ fbb: inout FlatBufferBuilder) { fbb.add(element: dataMode.rawValue, def: 0, at: VTOFFSET.dataMode.p) }
  public static func add(engine: Offset, _ fbb: inout FlatBufferBuilder) { fbb.add(offset: engine, at: VTOFFSET.engine.p) }
  public static func endStage_Full(_ fbb: inout FlatBufferBuilder, start: UOffset) -> Offset { let end = Offset(offset: fbb.endTable(at: start)); return end }
  public static func createStage_Full(
    ...
    let __start = Stage_Full.startStage_Full(&fbb)
    Stage_Full.add(id: id, &fbb)
    Stage_Full.add(idLaunchVehicle: idLaunchVehicle, &fbb)
    Stage_Full.add(idEngine: idEngine, &fbb)
    Stage_Full.add(classificationMarking: classificationMarking, &fbb)
    Stage_Full.add(stageNumber: stageNumber, &fbb)
    Stage_Full.add(type: type, &fbb)
    Stage_Full.add(manufacturerOrgId: manufacturerOrgId, &fbb)
    Stage_Full.add(numEngines: numEngines, &fbb)
    Stage_Full.add(length: length, &fbb)
    Stage_Full.add(diameter: diameter, &fbb)
    Stage_Full.add(mass: mass, &fbb)
    Stage_Full.add(vernier: vernier, &fbb)
    Stage_Full.add(numVernier: numVernier, &fbb)
    Stage_Full.add(mainEngineThrustSeaLevel: mainEngineThrustSeaLevel, &fbb)
    Stage_Full.add(mainEngineThrustVacuum: mainEngineThrustVacuum, &fbb)
    Stage_Full.add(thrustSeaLevel: thrustSeaLevel, &fbb)
    Stage_Full.add(thrustVacuum: thrustVacuum, &fbb)
    Stage_Full.add(burnTime: burnTime, &fbb)
    Stage_Full.add(numBurns: numBurns, &fbb)
    Stage_Full.add(vernierBurnTime: vernierBurnTime, &fbb)
    Stage_Full.add(vernierNumBurns: vernierNumBurns, &fbb)
    Stage_Full.add(vernierThrustSeaLevel: vernierThrustSeaLevel, &fbb)
    Stage_Full.add(vernierThrustVacuum: vernierThrustVacuum, &fbb)
    Stage_Full.add(numStageElements: numStageElements, &fbb)
    Stage_Full.add(reusable: reusable, &fbb)
    Stage_Full.add(restartable: restartable, &fbb)
    Stage_Full.add(controlThruster1: controlThruster1, &fbb)
    Stage_Full.add(numControlThruster1: numControlThruster1, &fbb)
    Stage_Full.add(controlThruster2: controlThruster2, &fbb)
    Stage_Full.add(numControlThruster2: numControlThruster2, &fbb)
    Stage_Full.addVectorOf(photoUrls: photoUrls, &fbb)
    Stage_Full.add(notes: notes, &fbb)
    Stage_Full.add(avionicsNotes: avionicsNotes, &fbb)
    Stage_Full.addVectorOf(tags: tags, &fbb)
    Stage_Full.add(createdAt: createdAt, &fbb)
    Stage_Full.add(createdBy: createdBy, &fbb)
    Stage_Full.add(updatedAt: updatedAt, &fbb)
    Stage_Full.add(updatedBy: updatedBy, &fbb)
    Stage_Full.add(source: source, &fbb)
    Stage_Full.add(origin: origin, &fbb)
    Stage_Full.add(origNetwork: origNetwork, &fbb)
    Stage_Full.add(dataMode: dataMode, &fbb)
    Stage_Full.add(engine: engine, &fbb)
    return Stage_Full.endStage_Full(&fbb, start: __start)
  }
  ...
  public static func verify<T>(_ verifier: inout Verifier, at position: Int, of type: T.Type) throws where T: Verifiable {
    ...
    try _v.visit(field: VTOFFSET.dataMode.p, fieldName: "dataMode", required: false, type: Stage_Full_dataMode_Enum.self)
