...
public struct EvacMedication_Full: FlatBufferObject, Verifiable {
  ...
  public static func finish(_ fbb: inout FlatBufferBuilder, end: Offset, prefix: Bool = false) { fbb.finish(offset: end, fileId: EvacMedication_Full.id, addPrefix: prefix) }
  ...
  public static func startEvacMedication_Full(_ fbb: inout FlatBufferBuilder) -> UOffset { fbb.startTable(with: 6) }
  ...
  public static func endEvacMedication_Full(_ fbb: inout FlatBufferBuilder, start: UOffset) -> Offset { let end = Offset(offset: fbb.endTable(at: start)); return end }
  public static func createEvacMedication_Full(
    ...
    let __start = EvacMedication_Full.startEvacMedication_Full(&fbb)
    EvacMedication_Full.add(type: type, &fbb)
    EvacMedication_Full.add(bodyPart: bodyPart, &fbb)
    EvacMedication_Full.add(dose: dose, &fbb)
    EvacMedication_Full.add(adminRoute: adminRoute, &fbb)
    EvacMedication_Full.add(time: time, &fbb)
    EvacMedication_Full.add(comments: comments, &fbb)
    return EvacMedication_Full.endEvacMedication_Full(&fbb, start: __start)
  }
