...
public struct RoutePoints_Full: FlatBufferObject, Verifiable {
  ...
  public static func finish(_ fbb: inout FlatBufferBuilder, end: Offset, prefix: Bool = false) { fbb.finish(offset: end, fileId: RoutePoints_Full.id, addPrefix: prefix) }
  ...
  public static func startRoutePoints_Full(_ fbb: inout FlatBufferBuilder) -> UOffset { fbb.startTable(with: 13) }
  ...
  public static func endRoutePoints_Full(_ fbb: inout FlatBufferBuilder, start: UOffset) -> Offset { let end = Offset(offset: fbb.endTable(at: start)); return end }
  public static func createRoutePoints_Full(
    ...
    let __start = RoutePoints_Full.startRoutePoints_Full(&fbb)
    RoutePoints_Full.add(ptSequenceId: ptSequenceId, &fbb)
    RoutePoints_Full.add(ptTypeCode: ptTypeCode, &fbb)
    RoutePoints_Full.add(ptTypeName: ptTypeName, &fbb)
    RoutePoints_Full.add(waypointName: waypointName, &fbb)
    RoutePoints_Full.add(dafifPt: dafifPt, &fbb)
    RoutePoints_Full.add(ptLat: ptLat, &fbb)
    RoutePoints_Full.add(ptLon: ptLon, &fbb)
    RoutePoints_Full.add(magDec: magDec, &fbb)
    RoutePoints_Full.add(navaidLength: navaidLength, &fbb)
    RoutePoints_Full.add(navaid: navaid, &fbb)
    RoutePoints_Full.add(countryCode: countryCode, &fbb)
    RoutePoints_Full.add(altCountryCode: altCountryCode, &fbb)
    RoutePoints_Full.add(navaidType: navaidType, &fbb)
    return RoutePoints_Full.endRoutePoints_Full(&fbb, start: __start)
  }
