...
public enum AIS_Full_dataMode_Enum: Int8, Enum, Verifiable {
  ...
  public static var max: AIS_Full_dataMode_Enum { return .exercise }
  public static var min: AIS_Full_dataMode_Enum { return .real }
}
...
public struct AIS_Full: FlatBufferObject, Verifiable {

  static func validateVersion() { FlatBuffersVersion_24_3_25() }
  public var __buffer: ByteBuffer! { return _accessor.bb }
  private var _accessor: Table

  public static var id: String { "AISF" } 
  public static func finish(_ fbb: inout FlatBufferBuilder, end: Offset, prefix: Bool = false) { fbb.finish(offset: end, fileId: AIS_Full.id, addPrefix: prefix) }
  ...
  public var dataMode: AIS_Full_dataMode_Enum { let o = _accessor.offset(VTOFFSET.dataMode.v); return o == 0 ? .real : AIS_Full_dataMode_Enum(rawValue: _accessor.readBuffer(of: Int8.self, at: o)) ?? .real }
  ///  The originating source network on which this record was created, auto-populated by the system.
  ///  Example: /// Example: ORIG
  ///  Constraints: Minimum length = 1, Maximum length = 32
  public var origNetwork: String? { let o = _accessor.offset(VTOFFSET.origNetwork.v); return o == 0 ? nil : _accessor.string(at: o) }
  public var origNetworkSegmentArray: [UInt8]? { return _accessor.getVector(at: VTOFFSET.origNetwork.v) }
  public static func startAIS_Full(_ fbb: inout FlatBufferBuilder) -> UOffset { fbb.startTable(with: 51) }
  ...
  public static func add(dataMode: AIS_Full_dataMode_Enum, _ fbb: inout FlatBufferBuilder) { fbb.add(element: dataMode.rawValue, def: 0, at: VTOFFSET.dataMode.p) }
  public static func add(origNetwork: Offset, _ fbb: inout FlatBufferBuilder) { fbb.add(offset: origNetwork, at: VTOFFSET.origNetwork.p) }
  public static func endAIS_Full(_ fbb: inout FlatBufferBuilder, start: UOffset) -> Offset { let end = Offset(offset: fbb.endTable(at: start)); return end }
  public static func createAIS_Full(
    ...
    let __start = AIS_Full.startAIS_Full(&fbb)
    AIS_Full.add(id: id, &fbb)
    AIS_Full.add(classificationMarking: classificationMarking, &fbb)
    AIS_Full.add(idTrack: idTrack, &fbb)
    AIS_Full.add(idVessel: idVessel, &fbb)
    AIS_Full.add(mmsi: mmsi, &fbb)
    AIS_Full.add(imon: imon, &fbb)
    AIS_Full.add(ts: ts, &fbb)
    AIS_Full.add(shipName: shipName, &fbb)
    AIS_Full.add(shipType: shipType, &fbb)
    AIS_Full.add(engagedIn: engagedIn, &fbb)
    AIS_Full.add(specialCraft: specialCraft, &fbb)
    AIS_Full.add(cargoType: cargoType, &fbb)
    AIS_Full.add(callSign: callSign, &fbb)
    AIS_Full.add(vesselFlag: vesselFlag, &fbb)
    AIS_Full.add(lat: lat, &fbb)
    AIS_Full.add(lon: lon, &fbb)
    AIS_Full.add(posHiAccuracy: posHiAccuracy, &fbb)
    AIS_Full.add(posHiLatency: posHiLatency, &fbb)
    AIS_Full.add(speed: speed, &fbb)
    AIS_Full.add(trueHeading: trueHeading, &fbb)
    AIS_Full.add(course: course, &fbb)
    AIS_Full.add(rateOfTurn: rateOfTurn, &fbb)
    AIS_Full.add(specialManeuver: specialManeuver, &fbb)
    AIS_Full.add(navStatus: navStatus, &fbb)
    AIS_Full.add(posDeviceType: posDeviceType, &fbb)
    AIS_Full.addVectorOf(antennaRefDimensions: antennaRefDimensions, &fbb)
    AIS_Full.add(length: length, &fbb)
    AIS_Full.add(width: width, &fbb)
    AIS_Full.add(draught: draught, &fbb)
    AIS_Full.add(shipDescription: shipDescription, &fbb)
    AIS_Full.add(destination: destination, &fbb)
    AIS_Full.add(destinationEta: destinationEta, &fbb)
    AIS_Full.add(currentPortGuid: currentPortGuid, &fbb)
    AIS_Full.add(currentPortLocode: currentPortLocode, &fbb)
    AIS_Full.add(lastPortGuid: lastPortGuid, &fbb)
    AIS_Full.add(lastPortLocode: lastPortLocode, &fbb)
    AIS_Full.add(nextPortGuid: nextPortGuid, &fbb)
    AIS_Full.add(nextPortLocode: nextPortLocode, &fbb)
    AIS_Full.add(etaCalculated: etaCalculated, &fbb)
    AIS_Full.add(etaUpdated: etaUpdated, &fbb)
    AIS_Full.add(distanceToGo: distanceToGo, &fbb)
    AIS_Full.add(distanceTravelled: distanceTravelled, &fbb)
    AIS_Full.add(avgSpeed: avgSpeed, &fbb)
    AIS_Full.add(maxSpeed: maxSpeed, &fbb)
    AIS_Full.add(createdAt: createdAt, &fbb)
    AIS_Full.add(createdBy: createdBy, &fbb)
    AIS_Full.add(source: source, &fbb)
    AIS_Full.add(sourceDl: sourceDl, &fbb)
    AIS_Full.add(origin: origin, &fbb)
    AIS_Full.add(dataMode: dataMode, &fbb)
    AIS_Full.add(origNetwork: origNetwork, &fbb)
    return AIS_Full.endAIS_Full(&fbb, start: __start)
  }
  ...
  public static func verify<T>(_ verifier: inout Verifier, at position: Int, of type: T.Type) throws where T: Verifiable {
    ...
    try _v.visit(field: VTOFFSET.dataMode.p, fieldName: "dataMode", required: false, type: AIS_Full_dataMode_Enum.self)
