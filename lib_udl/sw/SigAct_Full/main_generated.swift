...
public enum SigAct_Full_dataMode_Enum: Int8, Enum, Verifiable {
  ...
  public static var max: SigAct_Full_dataMode_Enum { return .exercise }
  public static var min: SigAct_Full_dataMode_Enum { return .real }
}
...
public struct SigAct_Full: FlatBufferObject, Verifiable {
  ...
  public static func finish(_ fbb: inout FlatBufferBuilder, end: Offset, prefix: Bool = false) { fbb.finish(offset: end, fileId: SigAct_Full.id, addPrefix: prefix) }
  ...
  public var dataMode: SigAct_Full_dataMode_Enum { let o = _accessor.offset(VTOFFSET.dataMode.v); return o == 0 ? .real : SigAct_Full_dataMode_Enum(rawValue: _accessor.readBuffer(of: Int8.self, at: o)) ?? .real }
  ///  The originating source network on which this record was created, auto-populated by the system.
  ///  Example: /// Example: ORIG
  ///  Constraints: Minimum length = 1, Maximum length = 32
  public var origNetwork: String? { let o = _accessor.offset(VTOFFSET.origNetwork.v); return o == 0 ? nil : _accessor.string(at: o) }
  public var origNetworkSegmentArray: [UInt8]? { return _accessor.getVector(at: VTOFFSET.origNetwork.v) }
  public static func startSigAct_Full(_ fbb: inout FlatBufferBuilder) -> UOffset { fbb.startTable(with: 83) }
  ...
  public static func add(dataMode: SigAct_Full_dataMode_Enum, _ fbb: inout FlatBufferBuilder) { fbb.add(element: dataMode.rawValue, def: 0, at: VTOFFSET.dataMode.p) }
  public static func add(origNetwork: Offset, _ fbb: inout FlatBufferBuilder) { fbb.add(offset: origNetwork, at: VTOFFSET.origNetwork.p) }
  public static func endSigAct_Full(_ fbb: inout FlatBufferBuilder, start: UOffset) -> Offset { let end = Offset(offset: fbb.endTable(at: start)); return end }
  public static func createSigAct_Full(
    ...
    let __start = SigAct_Full.startSigAct_Full(&fbb)
    SigAct_Full.add(id: id, &fbb)
    SigAct_Full.add(classificationMarking: classificationMarking, &fbb)
    SigAct_Full.add(sourceLanguage: sourceLanguage, &fbb)
    SigAct_Full.add(countryCode: countryCode, &fbb)
    SigAct_Full.add(city: city, &fbb)
    SigAct_Full.add(district: district, &fbb)
    SigAct_Full.add(province: province, &fbb)
    SigAct_Full.add(lat: lat, &fbb)
    SigAct_Full.add(lon: lon, &fbb)
    SigAct_Full.add(milgrid: milgrid, &fbb)
    SigAct_Full.add(theater: theater, &fbb)
    SigAct_Full.add(area: area, &fbb)
    SigAct_Full.add(atext: atext, &fbb)
    SigAct_Full.add(agjson: agjson, &fbb)
    SigAct_Full.add(atype: atype, &fbb)
    SigAct_Full.add(andims: andims, &fbb)
    SigAct_Full.add(asrid: asrid, &fbb)
    SigAct_Full.add(eventStart: eventStart, &fbb)
    SigAct_Full.add(eventEnd: eventEnd, &fbb)
    SigAct_Full.add(reportDate: reportDate, &fbb)
    SigAct_Full.addVectorOf(actors: actors, &fbb)
    SigAct_Full.add(civAbd: civAbd, &fbb)
    SigAct_Full.add(civDet: civDet, &fbb)
    SigAct_Full.add(civKia: civKia, &fbb)
    SigAct_Full.add(civWound: civWound, &fbb)
    SigAct_Full.add(coalAbd: coalAbd, &fbb)
    SigAct_Full.add(coalDet: coalDet, &fbb)
    SigAct_Full.add(coalKia: coalKia, &fbb)
    SigAct_Full.add(coalWound: coalWound, &fbb)
    SigAct_Full.add(enemyAbd: enemyAbd, &fbb)
    SigAct_Full.add(enemyDet: enemyDet, &fbb)
    SigAct_Full.add(enemyKia: enemyKia, &fbb)
    SigAct_Full.add(friendlyAbd: friendlyAbd, &fbb)
    SigAct_Full.add(friendlyDet: friendlyDet, &fbb)
    SigAct_Full.add(friendlyKia: friendlyKia, &fbb)
    SigAct_Full.add(friendlyWound: friendlyWound, &fbb)
    SigAct_Full.add(hostNatAbd: hostNatAbd, &fbb)
    SigAct_Full.add(hostNatDet: hostNatDet, &fbb)
    SigAct_Full.add(hostNatKia: hostNatKia, &fbb)
    SigAct_Full.add(hostNatWound: hostNatWound, &fbb)
    SigAct_Full.add(sideAabd: sideAabd, &fbb)
    SigAct_Full.add(sideAdet: sideAdet, &fbb)
    SigAct_Full.add(sideAkia: sideAkia, &fbb)
    SigAct_Full.add(sideAwound: sideAwound, &fbb)
    SigAct_Full.add(sideBabd: sideBabd, &fbb)
    SigAct_Full.add(sideBdet: sideBdet, &fbb)
    SigAct_Full.add(sideBkia: sideBkia, &fbb)
    SigAct_Full.add(sideBwound: sideBwound, &fbb)
    SigAct_Full.add(target: target, &fbb)
    SigAct_Full.add(eventDescription: eventDescription, &fbb)
    SigAct_Full.add(eventType: eventType, &fbb)
    SigAct_Full.add(typeOfAttack: typeOfAttack, &fbb)
    SigAct_Full.add(complexAttack: complexAttack, &fbb)
    SigAct_Full.add(idNumber: idNumber, &fbb)
    SigAct_Full.add(confidence: confidence, &fbb)
    SigAct_Full.add(accuracy: accuracy, &fbb)
    SigAct_Full.add(clarity: clarity, &fbb)
    SigAct_Full.add(cameoCode: cameoCode, &fbb)
    SigAct_Full.add(cameoBaseCode: cameoBaseCode, &fbb)
    SigAct_Full.add(cameoRootCode: cameoRootCode, &fbb)
    SigAct_Full.add(goldstein: goldstein, &fbb)
    SigAct_Full.add(documentSource: documentSource, &fbb)
    SigAct_Full.add(documentFilename: documentFilename, &fbb)
    SigAct_Full.add(hasAttachment: hasAttachment, &fbb)
    SigAct_Full.add(filesize: filesize, &fbb)
    SigAct_Full.add(checksumValue: checksumValue, &fbb)
    SigAct_Full.add(repUnit: repUnit, &fbb)
    SigAct_Full.add(repUnitType: repUnitType, &fbb)
    SigAct_Full.add(repUnitActivity: repUnitActivity, &fbb)
    SigAct_Full.add(numMentions: numMentions, &fbb)
    SigAct_Full.add(numSources: numSources, &fbb)
    SigAct_Full.add(numArticles: numArticles, &fbb)
    SigAct_Full.add(avgTone: avgTone, &fbb)
    SigAct_Full.add(sourceUrl: sourceUrl, &fbb)
    SigAct_Full.addVectorOf(relatedDocs: relatedDocs, &fbb)
    SigAct_Full.add(summary: summary, &fbb)
    SigAct_Full.add(notes: notes, &fbb)
    SigAct_Full.add(createdAt: createdAt, &fbb)
    SigAct_Full.add(createdBy: createdBy, &fbb)
    SigAct_Full.add(source: source, &fbb)
    SigAct_Full.add(origin: origin, &fbb)
    SigAct_Full.add(dataMode: dataMode, &fbb)
    SigAct_Full.add(origNetwork: origNetwork, &fbb)
    return SigAct_Full.endSigAct_Full(&fbb, start: __start)
  }
  ...
  public static func verify<T>(_ verifier: inout Verifier, at position: Int, of type: T.Type) throws where T: Verifiable {
    ...
    try _v.visit(field: VTOFFSET.dataMode.p, fieldName: "dataMode", required: false, type: SigAct_Full_dataMode_Enum.self)
