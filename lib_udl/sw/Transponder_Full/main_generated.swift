...
public enum Transponder_Full_dataMode_Enum: Int8, Enum, Verifiable {
  ...
  public static var max: Transponder_Full_dataMode_Enum { return .exercise }
  public static var min: Transponder_Full_dataMode_Enum { return .real }
}
...
public struct Transponder_Full: FlatBufferObject, Verifiable {
  ...
  public static func finish(_ fbb: inout FlatBufferBuilder, end: Offset, prefix: Bool = false) { fbb.finish(offset: end, fileId: Transponder_Full.id, addPrefix: prefix) }
  ...
  ///  Constraints: Minimum length = 1, Maximum length = 32
  public var dataMode: Transponder_Full_dataMode_Enum { let o = _accessor.offset(VTOFFSET.dataMode.v); return o == 0 ? .real : Transponder_Full_dataMode_Enum(rawValue: _accessor.readBuffer(of: Int8.self, at: o)) ?? .real }
  ...
  public static func startTransponder_Full(_ fbb: inout FlatBufferBuilder) -> UOffset { fbb.startTable(with: 21) }
  ...
  public static func add(dataMode: Transponder_Full_dataMode_Enum, _ fbb: inout FlatBufferBuilder) { fbb.add(element: dataMode.rawValue, def: 0, at: VTOFFSET.dataMode.p) }
  ...
  public static func endTransponder_Full(_ fbb: inout FlatBufferBuilder, start: UOffset) -> Offset { let end = Offset(offset: fbb.endTable(at: start)); return end }
  public static func createTransponder_Full(
    ...
    let __start = Transponder_Full.startTransponder_Full(&fbb)
    Transponder_Full.add(id: id, &fbb)
    Transponder_Full.add(idComm: idComm, &fbb)
    Transponder_Full.add(name: name, &fbb)
    Transponder_Full.add(classificationMarking: classificationMarking, &fbb)
    Transponder_Full.add(nid: nid, &fbb)
    Transponder_Full.add(tid: tid, &fbb)
    Transponder_Full.add(ttf: ttf, &fbb)
    Transponder_Full.add(symbolRate: symbolRate, &fbb)
    Transponder_Full.add(fec: fec, &fbb)
    Transponder_Full.add(modulation: modulation, &fbb)
    Transponder_Full.add(format: format, &fbb)
    Transponder_Full.add(system: system, &fbb)
    Transponder_Full.add(source: source, &fbb)
    Transponder_Full.add(origin: origin, &fbb)
    Transponder_Full.add(origNetwork: origNetwork, &fbb)
    Transponder_Full.add(dataMode: dataMode, &fbb)
    Transponder_Full.add(createdAt: createdAt, &fbb)
    Transponder_Full.add(createdBy: createdBy, &fbb)
    Transponder_Full.add(updatedAt: updatedAt, &fbb)
    Transponder_Full.add(updatedBy: updatedBy, &fbb)
    Transponder_Full.addVectorOf(channels: channels, &fbb)
    return Transponder_Full.endTransponder_Full(&fbb, start: __start)
  }
  ...
  public static func verify<T>(_ verifier: inout Verifier, at position: Int, of type: T.Type) throws where T: Verifiable {
    ...
    try _v.visit(field: VTOFFSET.dataMode.p, fieldName: "dataMode", required: false, type: Transponder_Full_dataMode_Enum.self)
