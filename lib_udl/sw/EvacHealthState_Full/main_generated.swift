...
public struct EvacHealthState_Full: FlatBufferObject, Verifiable {
  ...
  public static func finish(_ fbb: inout FlatBufferBuilder, end: Offset, prefix: Bool = false) { fbb.finish(offset: end, fileId: EvacHealthState_Full.id, addPrefix: prefix) }
  ...
  public static func startEvacHealthState_Full(_ fbb: inout FlatBufferBuilder) -> UOffset { fbb.startTable(with: 4) }
  ...
  public static func endEvacHealthState_Full(_ fbb: inout FlatBufferBuilder, start: UOffset) -> Offset { let end = Offset(offset: fbb.endTable(at: start)); return end }
  public static func createEvacHealthState_Full(
    ...
    let __start = EvacHealthState_Full.startEvacHealthState_Full(&fbb)
    EvacHealthState_Full.add(type: type, &fbb)
    EvacHealthState_Full.add(healthStateCode: healthStateCode, &fbb)
    EvacHealthState_Full.add(medConfFactor: medConfFactor, &fbb)
    EvacHealthState_Full.add(time: time, &fbb)
    return EvacHealthState_Full.endEvacHealthState_Full(&fbb, start: __start)
  }
