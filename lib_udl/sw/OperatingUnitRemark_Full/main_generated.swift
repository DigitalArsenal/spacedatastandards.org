...
public enum OperatingUnitRemark_Full_dataMode_Enum: Int8, Enum, Verifiable {
  ...
  public static var max: OperatingUnitRemark_Full_dataMode_Enum { return .exercise }
  public static var min: OperatingUnitRemark_Full_dataMode_Enum { return .real }
}
...
public struct OperatingUnitRemark_Full: FlatBufferObject, Verifiable {
  ...
  public static func finish(_ fbb: inout FlatBufferBuilder, end: Offset, prefix: Bool = false) { fbb.finish(offset: end, fileId: OperatingUnitRemark_Full.id, addPrefix: prefix) }
  ...
  public var dataMode: OperatingUnitRemark_Full_dataMode_Enum { let o = _accessor.offset(VTOFFSET.dataMode.v); return o == 0 ? .real : OperatingUnitRemark_Full_dataMode_Enum(rawValue: _accessor.readBuffer(of: Int8.self, at: o)) ?? .real }
  public static func startOperatingUnitRemark_Full(_ fbb: inout FlatBufferBuilder) -> UOffset { fbb.startTable(with: 15) }
  ...
  public static func add(dataMode: OperatingUnitRemark_Full_dataMode_Enum, _ fbb: inout FlatBufferBuilder) { fbb.add(element: dataMode.rawValue, def: 0, at: VTOFFSET.dataMode.p) }
  public static func endOperatingUnitRemark_Full(_ fbb: inout FlatBufferBuilder, start: UOffset) -> Offset { let end = Offset(offset: fbb.endTable(at: start)); return end }
  public static func createOperatingUnitRemark_Full(
    ...
    let __start = OperatingUnitRemark_Full.startOperatingUnitRemark_Full(&fbb)
    OperatingUnitRemark_Full.add(id: id, &fbb)
    OperatingUnitRemark_Full.add(classificationMarking: classificationMarking, &fbb)
    OperatingUnitRemark_Full.add(idOperatingUnit: idOperatingUnit, &fbb)
    OperatingUnitRemark_Full.add(altRmkId: altRmkId, &fbb)
    OperatingUnitRemark_Full.add(name: name, &fbb)
    OperatingUnitRemark_Full.add(type: type, &fbb)
    OperatingUnitRemark_Full.add(text: text, &fbb)
    OperatingUnitRemark_Full.add(code: code, &fbb)
    OperatingUnitRemark_Full.add(createdAt: createdAt, &fbb)
    OperatingUnitRemark_Full.add(createdBy: createdBy, &fbb)
    OperatingUnitRemark_Full.add(updatedAt: updatedAt, &fbb)
    OperatingUnitRemark_Full.add(updatedBy: updatedBy, &fbb)
    OperatingUnitRemark_Full.add(source: source, &fbb)
    OperatingUnitRemark_Full.add(origin: origin, &fbb)
    OperatingUnitRemark_Full.add(dataMode: dataMode, &fbb)
    return OperatingUnitRemark_Full.endOperatingUnitRemark_Full(&fbb, start: __start)
  }
  ...
  public static func verify<T>(_ verifier: inout Verifier, at position: Int, of type: T.Type) throws where T: Verifiable {
    ...
    try _v.visit(field: VTOFFSET.dataMode.p, fieldName: "dataMode", required: false, type: OperatingUnitRemark_Full_dataMode_Enum.self)
    _v.finish()
  }
}
