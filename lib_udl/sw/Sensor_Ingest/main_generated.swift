...
public enum Sensor_Ingest_dataMode_Enum: Int8, Enum, Verifiable {
  ...
  public static var max: Sensor_Ingest_dataMode_Enum { return .exercise }
  public static var min: Sensor_Ingest_dataMode_Enum { return .real }
}


///  /// Model representation of observation data for electro-optical based sensor phenomenologies.
public struct Sensor_Ingest: FlatBufferObject, Verifiable {
  ...
  public static func finish(_ fbb: inout FlatBufferBuilder, end: Offset, prefix: Bool = false) { fbb.finish(offset: end, fileId: Sensor_Ingest.id, addPrefix: prefix) }
  ...
  ///  Constraints: Minimum length = 1, Maximum length = 32
  public var dataMode: Sensor_Ingest_dataMode_Enum { let o = _accessor.offset(VTOFFSET.dataMode.v); return o == 0 ? .real : Sensor_Ingest_dataMode_Enum(rawValue: _accessor.readBuffer(of: Int8.self, at: o)) ?? .real }
  ...
  public static func startSensor_Ingest(_ fbb: inout FlatBufferBuilder) -> UOffset { fbb.startTable(with: 22) }
  ...
  public static func add(dataMode: Sensor_Ingest_dataMode_Enum, _ fbb: inout FlatBufferBuilder) { fbb.add(element: dataMode.rawValue, def: 0, at: VTOFFSET.dataMode.p) }
  ...
  public static func endSensor_Ingest(_ fbb: inout FlatBufferBuilder, start: UOffset) -> Offset { let end = Offset(offset: fbb.endTable(at: start)); return end }
  public static func createSensor_Ingest(
    ...
    let __start = Sensor_Ingest.startSensor_Ingest(&fbb)
    Sensor_Ingest.add(idSensor: idSensor, &fbb)
    Sensor_Ingest.add(sensorName: sensorName, &fbb)
    Sensor_Ingest.add(classificationMarking: classificationMarking, &fbb)
    Sensor_Ingest.add(sensorNumber: sensorNumber, &fbb)
    Sensor_Ingest.add(shortName: shortName, &fbb)
    Sensor_Ingest.add(dataControl: dataControl, &fbb)
    Sensor_Ingest.add(afId: afId, &fbb)
    Sensor_Ingest.add(asrType: asrType, &fbb)
    Sensor_Ingest.add(source: source, &fbb)
    Sensor_Ingest.add(origin: origin, &fbb)
    Sensor_Ingest.add(origNetwork: origNetwork, &fbb)
    Sensor_Ingest.add(dataMode: dataMode, &fbb)
    Sensor_Ingest.add(createdAt: createdAt, &fbb)
    Sensor_Ingest.add(createdBy: createdBy, &fbb)
    Sensor_Ingest.add(active: active, &fbb)
    Sensor_Ingest.addVectorOf(sensorcharacteristics: sensorcharacteristics, &fbb)
    Sensor_Ingest.addVectorOf(sensorStats: sensorStats, &fbb)
    Sensor_Ingest.addVectorOf(sensorlimitsCollection: sensorlimitsCollection, &fbb)
    Sensor_Ingest.add(entity: entity, &fbb)
    Sensor_Ingest.add(idEntity: idEntity, &fbb)
    Sensor_Ingest.add(sensorObservationType: sensorObservationType, &fbb)
    Sensor_Ingest.add(sensorType: sensorType, &fbb)
    return Sensor_Ingest.endSensor_Ingest(&fbb, start: __start)
  }
  ...
  public static func verify<T>(_ verifier: inout Verifier, at position: Int, of type: T.Type) throws where T: Verifiable {
    ...
    try _v.visit(field: VTOFFSET.dataMode.p, fieldName: "dataMode", required: false, type: Sensor_Ingest_dataMode_Enum.self)
