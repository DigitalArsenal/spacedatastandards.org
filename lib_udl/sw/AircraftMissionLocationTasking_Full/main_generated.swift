...
public struct AircraftMissionLocationTasking_Full: FlatBufferObject, Verifiable {
  ...
  public static func finish(_ fbb: inout FlatBufferBuilder, end: Offset, prefix: Bool = false) { fbb.finish(offset: end, fileId: AircraftMissionLocationTasking_Full.id, addPrefix: prefix) }
  ...
  public static func startAircraftMissionLocationTasking_Full(_ fbb: inout FlatBufferBuilder) -> UOffset { fbb.startTable(with: 10) }
  ...
  public static func endAircraftMissionLocationTasking_Full(_ fbb: inout FlatBufferBuilder, start: UOffset) -> Offset { let end = Offset(offset: fbb.endTable(at: start)); return end }
  public static func createAircraftMissionLocationTasking_Full(
    ...
    let __start = AircraftMissionLocationTasking_Full.startAircraftMissionLocationTasking_Full(&fbb)
    AircraftMissionLocationTasking_Full.add(startTime: startTime, &fbb)
    AircraftMissionLocationTasking_Full.add(endTime: endTime, &fbb)
    AircraftMissionLocationTasking_Full.add(msnLocName: msnLocName, &fbb)
    AircraftMissionLocationTasking_Full.add(alt: alt, &fbb)
    AircraftMissionLocationTasking_Full.add(airMsnPri: airMsnPri, &fbb)
    AircraftMissionLocationTasking_Full.add(areaGeoRad: areaGeoRad, &fbb)
    AircraftMissionLocationTasking_Full.add(msnLocPtName: msnLocPtName, &fbb)
    AircraftMissionLocationTasking_Full.add(msnLocPtLat: msnLocPtLat, &fbb)
    AircraftMissionLocationTasking_Full.add(msnLocPtLon: msnLocPtLon, &fbb)
    AircraftMissionLocationTasking_Full.add(msnLocPtBarT: msnLocPtBarT, &fbb)
    return AircraftMissionLocationTasking_Full.endAircraftMissionLocationTasking_Full(&fbb, start: __start)
  }
