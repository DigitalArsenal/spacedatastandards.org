// automatically generated by the FlatBuffers compiler, do not modify

/* eslint-disable @typescript-eslint/no-unused-vars, @typescript-eslint/no-explicit-any, @typescript-eslint/no-non-null-assertion */

import * as flatbuffers from 'flatbuffers';

import { Service, ServiceT } from './Service.js';


/**
 * Collection of services
 */
export class ServiceSet implements flatbuffers.IUnpackableObject<ServiceSetT> {
  bb: flatbuffers.ByteBuffer|null = null;
  bb_pos = 0;
  __init(i:number, bb:flatbuffers.ByteBuffer):ServiceSet {
  this.bb_pos = i;
  this.bb = bb;
  return this;
}

static getRootAsServiceSet(bb:flatbuffers.ByteBuffer, obj?:ServiceSet):ServiceSet {
  return (obj || new ServiceSet()).__init(bb.readInt32(bb.position()) + bb.position(), bb);
}

static getSizePrefixedRootAsServiceSet(bb:flatbuffers.ByteBuffer, obj?:ServiceSet):ServiceSet {
  bb.setPosition(bb.position() + flatbuffers.SIZE_PREFIX_LENGTH);
  return (obj || new ServiceSet()).__init(bb.readInt32(bb.position()) + bb.position(), bb);
}

/**
 * Services
 */
SERVICES(index: number, obj?:Service):Service|null {
  const offset = this.bb!.__offset(this.bb_pos, 4);
  return offset ? (obj || new Service()).__init(this.bb!.__indirect(this.bb!.__vector(this.bb_pos + offset) + index * 4), this.bb!) : null;
}

servicesLength():number {
  const offset = this.bb!.__offset(this.bb_pos, 4);
  return offset ? this.bb!.__vector_len(this.bb_pos + offset) : 0;
}

static startServiceSet(builder:flatbuffers.Builder) {
  builder.startObject(1);
}

static addServices(builder:flatbuffers.Builder, SERVICESOffset:flatbuffers.Offset) {
  builder.addFieldOffset(0, SERVICESOffset, 0);
}

static createServicesVector(builder:flatbuffers.Builder, data:flatbuffers.Offset[]):flatbuffers.Offset {
  builder.startVector(4, data.length, 4);
  for (let i = data.length - 1; i >= 0; i--) {
    builder.addOffset(data[i]!);
  }
  return builder.endVector();
}

static startServicesVector(builder:flatbuffers.Builder, numElems:number) {
  builder.startVector(4, numElems, 4);
}

static endServiceSet(builder:flatbuffers.Builder):flatbuffers.Offset {
  const offset = builder.endObject();
  return offset;
}

static createServiceSet(builder:flatbuffers.Builder, SERVICESOffset:flatbuffers.Offset):flatbuffers.Offset {
  ServiceSet.startServiceSet(builder);
  ServiceSet.addServices(builder, SERVICESOffset);
  return ServiceSet.endServiceSet(builder);
}

unpack(): ServiceSetT {
  return new ServiceSetT(
    this.bb!.createObjList<Service, ServiceT>(this.SERVICES.bind(this), this.servicesLength())
  );
}


unpackTo(_o: ServiceSetT): void {
  _o.SERVICES = this.bb!.createObjList<Service, ServiceT>(this.SERVICES.bind(this), this.servicesLength());
}
}

export class ServiceSetT implements flatbuffers.IGeneratedObject {
constructor(
  public SERVICES: (ServiceT)[] = []
){}


pack(builder:flatbuffers.Builder): flatbuffers.Offset {
  const SERVICES = ServiceSet.createServicesVector(builder, builder.createObjectOffsetList(this.SERVICES));

  return ServiceSet.createServiceSet(builder,
    SERVICES
  );
}
}
