// automatically generated by the FlatBuffers compiler, do not modify

/* eslint-disable @typescript-eslint/no-unused-vars, @typescript-eslint/no-explicit-any, @typescript-eslint/no-non-null-assertion */

import * as flatbuffers from 'flatbuffers';

import { KMLAltitudeMode } from './KMLAltitudeMode.js';
import { KMLCoordinate, KMLCoordinateT } from './KMLCoordinate.js';


/**
 * Point geometry
 */
export class KMLPoint implements flatbuffers.IUnpackableObject<KMLPointT> {
  bb: flatbuffers.ByteBuffer|null = null;
  bb_pos = 0;
  __init(i:number, bb:flatbuffers.ByteBuffer):KMLPoint {
  this.bb_pos = i;
  this.bb = bb;
  return this;
}

static getRootAsKMLPoint(bb:flatbuffers.ByteBuffer, obj?:KMLPoint):KMLPoint {
  return (obj || new KMLPoint()).__init(bb.readInt32(bb.position()) + bb.position(), bb);
}

static getSizePrefixedRootAsKMLPoint(bb:flatbuffers.ByteBuffer, obj?:KMLPoint):KMLPoint {
  bb.setPosition(bb.position() + flatbuffers.SIZE_PREFIX_LENGTH);
  return (obj || new KMLPoint()).__init(bb.readInt32(bb.position()) + bb.position(), bb);
}

/**
 * Coordinate
 */
COORDINATES(obj?:KMLCoordinate):KMLCoordinate|null {
  const offset = this.bb!.__offset(this.bb_pos, 4);
  return offset ? (obj || new KMLCoordinate()).__init(this.bb!.__indirect(this.bb_pos + offset), this.bb!) : null;
}

/**
 * Altitude mode
 */
ALTITUDE_MODE():KMLAltitudeMode {
  const offset = this.bb!.__offset(this.bb_pos, 6);
  return offset ? this.bb!.readInt8(this.bb_pos + offset) : KMLAltitudeMode.CLAMP_TO_GROUND;
}

/**
 * Whether to extrude to ground
 */
EXTRUDE():boolean {
  const offset = this.bb!.__offset(this.bb_pos, 8);
  return offset ? !!this.bb!.readInt8(this.bb_pos + offset) : false;
}

static startKMLPoint(builder:flatbuffers.Builder) {
  builder.startObject(3);
}

static addCoordinates(builder:flatbuffers.Builder, COORDINATESOffset:flatbuffers.Offset) {
  builder.addFieldOffset(0, COORDINATESOffset, 0);
}

static addAltitudeMode(builder:flatbuffers.Builder, ALTITUDE_MODE:KMLAltitudeMode) {
  builder.addFieldInt8(1, ALTITUDE_MODE, KMLAltitudeMode.CLAMP_TO_GROUND);
}

static addExtrude(builder:flatbuffers.Builder, EXTRUDE:boolean) {
  builder.addFieldInt8(2, +EXTRUDE, +false);
}

static endKMLPoint(builder:flatbuffers.Builder):flatbuffers.Offset {
  const offset = builder.endObject();
  return offset;
}

static createKMLPoint(builder:flatbuffers.Builder, COORDINATESOffset:flatbuffers.Offset, ALTITUDE_MODE:KMLAltitudeMode, EXTRUDE:boolean):flatbuffers.Offset {
  KMLPoint.startKMLPoint(builder);
  KMLPoint.addCoordinates(builder, COORDINATESOffset);
  KMLPoint.addAltitudeMode(builder, ALTITUDE_MODE);
  KMLPoint.addExtrude(builder, EXTRUDE);
  return KMLPoint.endKMLPoint(builder);
}

unpack(): KMLPointT {
  return new KMLPointT(
    (this.COORDINATES() !== null ? this.COORDINATES()!.unpack() : null),
    this.ALTITUDE_MODE(),
    this.EXTRUDE()
  );
}


unpackTo(_o: KMLPointT): void {
  _o.COORDINATES = (this.COORDINATES() !== null ? this.COORDINATES()!.unpack() : null);
  _o.ALTITUDE_MODE = this.ALTITUDE_MODE();
  _o.EXTRUDE = this.EXTRUDE();
}
}

export class KMLPointT implements flatbuffers.IGeneratedObject {
constructor(
  public COORDINATES: KMLCoordinateT|null = null,
  public ALTITUDE_MODE: KMLAltitudeMode = KMLAltitudeMode.CLAMP_TO_GROUND,
  public EXTRUDE: boolean = false
){}


pack(builder:flatbuffers.Builder): flatbuffers.Offset {
  const COORDINATES = (this.COORDINATES !== null ? this.COORDINATES!.pack(builder) : 0);

  return KMLPoint.createKMLPoint(builder,
    COORDINATES,
    this.ALTITUDE_MODE,
    this.EXTRUDE
  );
}
}
