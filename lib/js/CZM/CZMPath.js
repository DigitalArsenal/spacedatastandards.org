// automatically generated by the FlatBuffers compiler, do not modify
/* eslint-disable @typescript-eslint/no-unused-vars, @typescript-eslint/no-explicit-any, @typescript-eslint/no-non-null-assertion */
import * as flatbuffers from 'flatbuffers';
import { CZMColor } from './CZMColor.js';
import { CZMPolylineMaterial } from './CZMPolylineMaterial.js';
/**
 * Path properties (orbit trail)
 */
export class CZMPath {
    constructor() {
        this.bb = null;
        this.bb_pos = 0;
    }
    __init(i, bb) {
        this.bb_pos = i;
        this.bb = bb;
        return this;
    }
    static getRootAsCZMPath(bb, obj) {
        return (obj || new CZMPath()).__init(bb.readInt32(bb.position()) + bb.position(), bb);
    }
    static getSizePrefixedRootAsCZMPath(bb, obj) {
        bb.setPosition(bb.position() + flatbuffers.SIZE_PREFIX_LENGTH);
        return (obj || new CZMPath()).__init(bb.readInt32(bb.position()) + bb.position(), bb);
    }
    /**
     * Whether the path is displayed
     */
    SHOW() {
        const offset = this.bb.__offset(this.bb_pos, 4);
        return offset ? !!this.bb.readInt8(this.bb_pos + offset) : false;
    }
    /**
     * Trail time in seconds (how far behind)
     */
    LEAD_TIME() {
        const offset = this.bb.__offset(this.bb_pos, 6);
        return offset ? this.bb.readFloat64(this.bb_pos + offset) : 0.0;
    }
    /**
     * Lead time in seconds (how far ahead)
     */
    TRAIL_TIME() {
        const offset = this.bb.__offset(this.bb_pos, 8);
        return offset ? this.bb.readFloat64(this.bb_pos + offset) : 0.0;
    }
    /**
     * Line width in pixels
     */
    WIDTH() {
        const offset = this.bb.__offset(this.bb_pos, 10);
        return offset ? this.bb.readFloat64(this.bb_pos + offset) : 0.0;
    }
    /**
     * Path color (legacy solid color)
     */
    COLOR(obj) {
        const offset = this.bb.__offset(this.bb_pos, 12);
        return offset ? (obj || new CZMColor()).__init(this.bb.__indirect(this.bb_pos + offset), this.bb) : null;
    }
    /**
     * Resolution in seconds
     */
    RESOLUTION() {
        const offset = this.bb.__offset(this.bb_pos, 14);
        return offset ? this.bb.readFloat64(this.bb_pos + offset) : 0.0;
    }
    /**
     * Full polyline material
     */
    MATERIAL(obj) {
        const offset = this.bb.__offset(this.bb_pos, 16);
        return offset ? (obj || new CZMPolylineMaterial()).__init(this.bb.__indirect(this.bb_pos + offset), this.bb) : null;
    }
    static startCZMPath(builder) {
        builder.startObject(7);
    }
    static addShow(builder, SHOW) {
        builder.addFieldInt8(0, +SHOW, +false);
    }
    static addLeadTime(builder, LEAD_TIME) {
        builder.addFieldFloat64(1, LEAD_TIME, 0.0);
    }
    static addTrailTime(builder, TRAIL_TIME) {
        builder.addFieldFloat64(2, TRAIL_TIME, 0.0);
    }
    static addWidth(builder, WIDTH) {
        builder.addFieldFloat64(3, WIDTH, 0.0);
    }
    static addColor(builder, COLOROffset) {
        builder.addFieldOffset(4, COLOROffset, 0);
    }
    static addResolution(builder, RESOLUTION) {
        builder.addFieldFloat64(5, RESOLUTION, 0.0);
    }
    static addMaterial(builder, MATERIALOffset) {
        builder.addFieldOffset(6, MATERIALOffset, 0);
    }
    static endCZMPath(builder) {
        const offset = builder.endObject();
        return offset;
    }
    unpack() {
        return new CZMPathT(this.SHOW(), this.LEAD_TIME(), this.TRAIL_TIME(), this.WIDTH(), (this.COLOR() !== null ? this.COLOR().unpack() : null), this.RESOLUTION(), (this.MATERIAL() !== null ? this.MATERIAL().unpack() : null));
    }
    unpackTo(_o) {
        _o.SHOW = this.SHOW();
        _o.LEAD_TIME = this.LEAD_TIME();
        _o.TRAIL_TIME = this.TRAIL_TIME();
        _o.WIDTH = this.WIDTH();
        _o.COLOR = (this.COLOR() !== null ? this.COLOR().unpack() : null);
        _o.RESOLUTION = this.RESOLUTION();
        _o.MATERIAL = (this.MATERIAL() !== null ? this.MATERIAL().unpack() : null);
    }
}
export class CZMPathT {
    constructor(SHOW = false, LEAD_TIME = 0.0, TRAIL_TIME = 0.0, WIDTH = 0.0, COLOR = null, RESOLUTION = 0.0, MATERIAL = null) {
        this.SHOW = SHOW;
        this.LEAD_TIME = LEAD_TIME;
        this.TRAIL_TIME = TRAIL_TIME;
        this.WIDTH = WIDTH;
        this.COLOR = COLOR;
        this.RESOLUTION = RESOLUTION;
        this.MATERIAL = MATERIAL;
    }
    pack(builder) {
        const COLOR = (this.COLOR !== null ? this.COLOR.pack(builder) : 0);
        const MATERIAL = (this.MATERIAL !== null ? this.MATERIAL.pack(builder) : 0);
        CZMPath.startCZMPath(builder);
        CZMPath.addShow(builder, this.SHOW);
        CZMPath.addLeadTime(builder, this.LEAD_TIME);
        CZMPath.addTrailTime(builder, this.TRAIL_TIME);
        CZMPath.addWidth(builder, this.WIDTH);
        CZMPath.addColor(builder, COLOR);
        CZMPath.addResolution(builder, this.RESOLUTION);
        CZMPath.addMaterial(builder, MATERIAL);
        return CZMPath.endCZMPath(builder);
    }
}
