// automatically generated by the FlatBuffers compiler, do not modify
/* eslint-disable @typescript-eslint/no-unused-vars, @typescript-eslint/no-explicit-any, @typescript-eslint/no-non-null-assertion */
import * as flatbuffers from 'flatbuffers';
import { SCVVec3 } from './SCVVec3.js';
import { scvCoordinateFrame } from './scvCoordinateFrame.js';
import { scvGeometryDomain } from './scvGeometryDomain.js';
import { scvTargetShape } from './scvTargetShape.js';
export class SCVTarget {
    constructor() {
        this.bb = null;
        this.bb_pos = 0;
    }
    __init(i, bb) {
        this.bb_pos = i;
        this.bb = bb;
        return this;
    }
    static getRootAsSCVTarget(bb, obj) {
        return (obj || new SCVTarget()).__init(bb.readInt32(bb.position()) + bb.position(), bb);
    }
    static getSizePrefixedRootAsSCVTarget(bb, obj) {
        bb.setPosition(bb.position() + flatbuffers.SIZE_PREFIX_LENGTH);
        return (obj || new SCVTarget()).__init(bb.readInt32(bb.position()) + bb.position(), bb);
    }
    TARGET_ID() {
        const offset = this.bb.__offset(this.bb_pos, 4);
        return offset ? this.bb.readUint32(this.bb_pos + offset) : 0;
    }
    OBJECT_ID(optionalEncoding) {
        const offset = this.bb.__offset(this.bb_pos, 6);
        return offset ? this.bb.__string(this.bb_pos + offset, optionalEncoding) : null;
    }
    NAME(optionalEncoding) {
        const offset = this.bb.__offset(this.bb_pos, 8);
        return offset ? this.bb.__string(this.bb_pos + offset, optionalEncoding) : null;
    }
    FRAME() {
        const offset = this.bb.__offset(this.bb_pos, 10);
        return offset ? this.bb.readUint8(this.bb_pos + offset) : scvCoordinateFrame.UNKNOWN;
    }
    POSITION_M(obj) {
        const offset = this.bb.__offset(this.bb_pos, 12);
        return offset ? (obj || new SCVVec3()).__init(this.bb.__indirect(this.bb_pos + offset), this.bb) : null;
    }
    VELOCITY_MPS(obj) {
        const offset = this.bb.__offset(this.bb_pos, 14);
        return offset ? (obj || new SCVVec3()).__init(this.bb.__indirect(this.bb_pos + offset), this.bb) : null;
    }
    RADIUS_M() {
        const offset = this.bb.__offset(this.bb_pos, 16);
        return offset ? this.bb.readFloat64(this.bb_pos + offset) : 0.0;
    }
    TARGET_KIND() {
        const offset = this.bb.__offset(this.bb_pos, 18);
        return offset ? this.bb.readUint8(this.bb_pos + offset) : scvTargetShape.POINT;
    }
    DOMAIN() {
        const offset = this.bb.__offset(this.bb_pos, 20);
        return offset ? this.bb.readUint8(this.bb_pos + offset) : scvGeometryDomain.SURFACE;
    }
    POLYGON_VERTICES(index, obj) {
        const offset = this.bb.__offset(this.bb_pos, 22);
        return offset ? (obj || new SCVVec3()).__init(this.bb.__indirect(this.bb.__vector(this.bb_pos + offset) + index * 4), this.bb) : null;
    }
    polygonVerticesLength() {
        const offset = this.bb.__offset(this.bb_pos, 22);
        return offset ? this.bb.__vector_len(this.bb_pos + offset) : 0;
    }
    MIN_ALTITUDE_M() {
        const offset = this.bb.__offset(this.bb_pos, 24);
        return offset ? this.bb.readFloat64(this.bb_pos + offset) : 0.0;
    }
    MAX_ALTITUDE_M() {
        const offset = this.bb.__offset(this.bb_pos, 26);
        return offset ? this.bb.readFloat64(this.bb_pos + offset) : 0.0;
    }
    static startSCVTarget(builder) {
        builder.startObject(12);
    }
    static addTargetId(builder, TARGET_ID) {
        builder.addFieldInt32(0, TARGET_ID, 0);
    }
    static addObjectId(builder, OBJECT_IDOffset) {
        builder.addFieldOffset(1, OBJECT_IDOffset, 0);
    }
    static addName(builder, NAMEOffset) {
        builder.addFieldOffset(2, NAMEOffset, 0);
    }
    static addFrame(builder, FRAME) {
        builder.addFieldInt8(3, FRAME, scvCoordinateFrame.UNKNOWN);
    }
    static addPositionM(builder, POSITION_MOffset) {
        builder.addFieldOffset(4, POSITION_MOffset, 0);
    }
    static addVelocityMps(builder, VELOCITY_MPSOffset) {
        builder.addFieldOffset(5, VELOCITY_MPSOffset, 0);
    }
    static addRadiusM(builder, RADIUS_M) {
        builder.addFieldFloat64(6, RADIUS_M, 0.0);
    }
    static addTargetKind(builder, TARGET_KIND) {
        builder.addFieldInt8(7, TARGET_KIND, scvTargetShape.POINT);
    }
    static addDomain(builder, DOMAIN) {
        builder.addFieldInt8(8, DOMAIN, scvGeometryDomain.SURFACE);
    }
    static addPolygonVertices(builder, POLYGON_VERTICESOffset) {
        builder.addFieldOffset(9, POLYGON_VERTICESOffset, 0);
    }
    static createPolygonVerticesVector(builder, data) {
        builder.startVector(4, data.length, 4);
        for (let i = data.length - 1; i >= 0; i--) {
            builder.addOffset(data[i]);
        }
        return builder.endVector();
    }
    static startPolygonVerticesVector(builder, numElems) {
        builder.startVector(4, numElems, 4);
    }
    static addMinAltitudeM(builder, MIN_ALTITUDE_M) {
        builder.addFieldFloat64(10, MIN_ALTITUDE_M, 0.0);
    }
    static addMaxAltitudeM(builder, MAX_ALTITUDE_M) {
        builder.addFieldFloat64(11, MAX_ALTITUDE_M, 0.0);
    }
    static endSCVTarget(builder) {
        const offset = builder.endObject();
        return offset;
    }
    unpack() {
        return new SCVTargetT(this.TARGET_ID(), this.OBJECT_ID(), this.NAME(), this.FRAME(), (this.POSITION_M() !== null ? this.POSITION_M().unpack() : null), (this.VELOCITY_MPS() !== null ? this.VELOCITY_MPS().unpack() : null), this.RADIUS_M(), this.TARGET_KIND(), this.DOMAIN(), this.bb.createObjList(this.POLYGON_VERTICES.bind(this), this.polygonVerticesLength()), this.MIN_ALTITUDE_M(), this.MAX_ALTITUDE_M());
    }
    unpackTo(_o) {
        _o.TARGET_ID = this.TARGET_ID();
        _o.OBJECT_ID = this.OBJECT_ID();
        _o.NAME = this.NAME();
        _o.FRAME = this.FRAME();
        _o.POSITION_M = (this.POSITION_M() !== null ? this.POSITION_M().unpack() : null);
        _o.VELOCITY_MPS = (this.VELOCITY_MPS() !== null ? this.VELOCITY_MPS().unpack() : null);
        _o.RADIUS_M = this.RADIUS_M();
        _o.TARGET_KIND = this.TARGET_KIND();
        _o.DOMAIN = this.DOMAIN();
        _o.POLYGON_VERTICES = this.bb.createObjList(this.POLYGON_VERTICES.bind(this), this.polygonVerticesLength());
        _o.MIN_ALTITUDE_M = this.MIN_ALTITUDE_M();
        _o.MAX_ALTITUDE_M = this.MAX_ALTITUDE_M();
    }
}
export class SCVTargetT {
    constructor(TARGET_ID = 0, OBJECT_ID = null, NAME = null, FRAME = scvCoordinateFrame.UNKNOWN, POSITION_M = null, VELOCITY_MPS = null, RADIUS_M = 0.0, TARGET_KIND = scvTargetShape.POINT, DOMAIN = scvGeometryDomain.SURFACE, POLYGON_VERTICES = [], MIN_ALTITUDE_M = 0.0, MAX_ALTITUDE_M = 0.0) {
        this.TARGET_ID = TARGET_ID;
        this.OBJECT_ID = OBJECT_ID;
        this.NAME = NAME;
        this.FRAME = FRAME;
        this.POSITION_M = POSITION_M;
        this.VELOCITY_MPS = VELOCITY_MPS;
        this.RADIUS_M = RADIUS_M;
        this.TARGET_KIND = TARGET_KIND;
        this.DOMAIN = DOMAIN;
        this.POLYGON_VERTICES = POLYGON_VERTICES;
        this.MIN_ALTITUDE_M = MIN_ALTITUDE_M;
        this.MAX_ALTITUDE_M = MAX_ALTITUDE_M;
    }
    pack(builder) {
        const OBJECT_ID = (this.OBJECT_ID !== null ? builder.createString(this.OBJECT_ID) : 0);
        const NAME = (this.NAME !== null ? builder.createString(this.NAME) : 0);
        const POSITION_M = (this.POSITION_M !== null ? this.POSITION_M.pack(builder) : 0);
        const VELOCITY_MPS = (this.VELOCITY_MPS !== null ? this.VELOCITY_MPS.pack(builder) : 0);
        const POLYGON_VERTICES = SCVTarget.createPolygonVerticesVector(builder, builder.createObjectOffsetList(this.POLYGON_VERTICES));
        SCVTarget.startSCVTarget(builder);
        SCVTarget.addTargetId(builder, this.TARGET_ID);
        SCVTarget.addObjectId(builder, OBJECT_ID);
        SCVTarget.addName(builder, NAME);
        SCVTarget.addFrame(builder, this.FRAME);
        SCVTarget.addPositionM(builder, POSITION_M);
        SCVTarget.addVelocityMps(builder, VELOCITY_MPS);
        SCVTarget.addRadiusM(builder, this.RADIUS_M);
        SCVTarget.addTargetKind(builder, this.TARGET_KIND);
        SCVTarget.addDomain(builder, this.DOMAIN);
        SCVTarget.addPolygonVertices(builder, POLYGON_VERTICES);
        SCVTarget.addMinAltitudeM(builder, this.MIN_ALTITUDE_M);
        SCVTarget.addMaxAltitudeM(builder, this.MAX_ALTITUDE_M);
        return SCVTarget.endSCVTarget(builder);
    }
}
