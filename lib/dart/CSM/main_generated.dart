...
class CSMCOLLECTION {
  CSMCOLLECTION._(this._bc, this._bcOffset);
  factory CSMCOLLECTION(List<int> bytes) {
    final rootRef = fb.BufferContext.fromBytes(bytes);
    return reader.read(rootRef, 0);
  }

  static const fb.Reader<CSMCOLLECTION> reader = _CSMCOLLECTIONReader();

  final fb.BufferContext _bc;
  final int _bcOffset;

  List<CSM>? get RECORDS => const fb.ListReader<CSM>(CSM.reader).vTableGetNullable(_bc, _bcOffset, 4);

  @override
  String toString() {
    return 'CSMCOLLECTION{RECORDS: ${RECORDS}}';
  }
}

class _CSMCOLLECTIONReader extends fb.TableReader<CSMCOLLECTION> {
  const _CSMCOLLECTIONReader();

  @override
  CSMCOLLECTION createObject(fb.BufferContext bc, int offset) => 
    CSMCOLLECTION._(bc, offset);
}

class CSMCOLLECTIONBuilder {
  CSMCOLLECTIONBuilder(this.fbBuilder);
...
class CSMCOLLECTIONObjectBuilder extends fb.ObjectBuilder {
  final List<CSMObjectBuilder>? _RECORDS;

  CSMCOLLECTIONObjectBuilder({
    List<CSMObjectBuilder>? RECORDS,
  })
      : _RECORDS = RECORDS;
