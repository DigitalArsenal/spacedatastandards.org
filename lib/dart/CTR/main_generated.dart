...
class CTRCOLLECTION {
  CTRCOLLECTION._(this._bc, this._bcOffset);
  factory CTRCOLLECTION(List<int> bytes) {
    final rootRef = fb.BufferContext.fromBytes(bytes);
    return reader.read(rootRef, 0);
  }

  static const fb.Reader<CTRCOLLECTION> reader = _CTRCOLLECTIONReader();

  final fb.BufferContext _bc;
  final int _bcOffset;

  List<CTR>? get RECORDS => const fb.ListReader<CTR>(CTR.reader).vTableGetNullable(_bc, _bcOffset, 4);

  @override
  String toString() {
    return 'CTRCOLLECTION{RECORDS: ${RECORDS}}';
  }
}

class _CTRCOLLECTIONReader extends fb.TableReader<CTRCOLLECTION> {
  const _CTRCOLLECTIONReader();

  @override
  CTRCOLLECTION createObject(fb.BufferContext bc, int offset) => 
    CTRCOLLECTION._(bc, offset);
}

class CTRCOLLECTIONBuilder {
  CTRCOLLECTIONBuilder(this.fbBuilder);
...
class CTRCOLLECTIONObjectBuilder extends fb.ObjectBuilder {
  final List<CTRObjectBuilder>? _RECORDS;

  CTRCOLLECTIONObjectBuilder({
    List<CTRObjectBuilder>? RECORDS,
  })
      : _RECORDS = RECORDS;
