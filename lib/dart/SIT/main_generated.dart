...
class SITCOLLECTION {
  SITCOLLECTION._(this._bc, this._bcOffset);
  factory SITCOLLECTION(List<int> bytes) {
    final rootRef = fb.BufferContext.fromBytes(bytes);
    return reader.read(rootRef, 0);
  }

  static const fb.Reader<SITCOLLECTION> reader = _SITCOLLECTIONReader();

  final fb.BufferContext _bc;
  final int _bcOffset;

  List<SIT>? get RECORDS => const fb.ListReader<SIT>(SIT.reader).vTableGetNullable(_bc, _bcOffset, 4);

  @override
  String toString() {
    return 'SITCOLLECTION{RECORDS: ${RECORDS}}';
  }
}

class _SITCOLLECTIONReader extends fb.TableReader<SITCOLLECTION> {
  const _SITCOLLECTIONReader();

  @override
  SITCOLLECTION createObject(fb.BufferContext bc, int offset) => 
    SITCOLLECTION._(bc, offset);
}

class SITCOLLECTIONBuilder {
  SITCOLLECTIONBuilder(this.fbBuilder);
...
class SITCOLLECTIONObjectBuilder extends fb.ObjectBuilder {
  final List<SITObjectBuilder>? _RECORDS;

  SITCOLLECTIONObjectBuilder({
    List<SITObjectBuilder>? RECORDS,
  })
      : _RECORDS = RECORDS;
