...
class VCM_COLLECTION {
  VCM_COLLECTION._(this._bc, this._bcOffset);
  factory VCM_COLLECTION(List<int> bytes) {
    final rootRef = fb.BufferContext.fromBytes(bytes);
    return reader.read(rootRef, 0);
  }

  static const fb.Reader<VCM_COLLECTION> reader = _VCM_COLLECTIONReader();

  final fb.BufferContext _bc;
  final int _bcOffset;

  List<VCM>? get RECORDS => const fb.ListReader<VCM>(VCM.reader).vTableGetNullable(_bc, _bcOffset, 4);

  @override
  String toString() {
    return 'VCM_COLLECTION{RECORDS: ${RECORDS}}';
  }
}

class _VCM_COLLECTIONReader extends fb.TableReader<VCM_COLLECTION> {
  const _VCM_COLLECTIONReader();

  @override
  VCM_COLLECTION createObject(fb.BufferContext bc, int offset) => 
    VCM_COLLECTION._(bc, offset);
}

class VCM_COLLECTIONBuilder {
  VCM_COLLECTIONBuilder(this.fbBuilder);
...
class VCM_COLLECTIONObjectBuilder extends fb.ObjectBuilder {
  final List<VCMObjectBuilder>? _RECORDS;

  VCM_COLLECTIONObjectBuilder({
    List<VCMObjectBuilder>? RECORDS,
  })
      : _RECORDS = RECORDS;
