...
class RFMCOLLECTION {
  RFMCOLLECTION._(this._bc, this._bcOffset);
  factory RFMCOLLECTION(List<int> bytes) {
    final rootRef = fb.BufferContext.fromBytes(bytes);
    return reader.read(rootRef, 0);
  }

  static const fb.Reader<RFMCOLLECTION> reader = _RFMCOLLECTIONReader();

  final fb.BufferContext _bc;
  final int _bcOffset;

  List<RFM>? get RECORDS => const fb.ListReader<RFM>(RFM.reader).vTableGetNullable(_bc, _bcOffset, 4);

  @override
  String toString() {
    return 'RFMCOLLECTION{RECORDS: ${RECORDS}}';
  }
}

class _RFMCOLLECTIONReader extends fb.TableReader<RFMCOLLECTION> {
  const _RFMCOLLECTIONReader();

  @override
  RFMCOLLECTION createObject(fb.BufferContext bc, int offset) => 
    RFMCOLLECTION._(bc, offset);
}

class RFMCOLLECTIONBuilder {
  RFMCOLLECTIONBuilder(this.fbBuilder);
...
class RFMCOLLECTIONObjectBuilder extends fb.ObjectBuilder {
  final List<RFMObjectBuilder>? _RECORDS;

  RFMCOLLECTIONObjectBuilder({
    List<RFMObjectBuilder>? RECORDS,
  })
      : _RECORDS = RECORDS;
