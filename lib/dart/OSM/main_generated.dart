...
class OSMCOLLECTION {
  OSMCOLLECTION._(this._bc, this._bcOffset);
  factory OSMCOLLECTION(List<int> bytes) {
    final rootRef = fb.BufferContext.fromBytes(bytes);
    return reader.read(rootRef, 0);
  }

  static const fb.Reader<OSMCOLLECTION> reader = _OSMCOLLECTIONReader();

  final fb.BufferContext _bc;
  final int _bcOffset;

  List<OSM>? get RECORDS => const fb.ListReader<OSM>(OSM.reader).vTableGetNullable(_bc, _bcOffset, 4);

  @override
  String toString() {
    return 'OSMCOLLECTION{RECORDS: ${RECORDS}}';
  }
}

class _OSMCOLLECTIONReader extends fb.TableReader<OSMCOLLECTION> {
  const _OSMCOLLECTIONReader();

  @override
  OSMCOLLECTION createObject(fb.BufferContext bc, int offset) => 
    OSMCOLLECTION._(bc, offset);
}

class OSMCOLLECTIONBuilder {
  OSMCOLLECTIONBuilder(this.fbBuilder);
...
class OSMCOLLECTIONObjectBuilder extends fb.ObjectBuilder {
  final List<OSMObjectBuilder>? _RECORDS;

  OSMCOLLECTIONObjectBuilder({
    List<OSMObjectBuilder>? RECORDS,
  })
      : _RECORDS = RECORDS;
