...
class BOVCOLLECTION {
  BOVCOLLECTION._(this._bc, this._bcOffset);
  factory BOVCOLLECTION(List<int> bytes) {
    final rootRef = fb.BufferContext.fromBytes(bytes);
    return reader.read(rootRef, 0);
  }

  static const fb.Reader<BOVCOLLECTION> reader = _BOVCOLLECTIONReader();

  final fb.BufferContext _bc;
  final int _bcOffset;

  List<BOV>? get RECORDS => const fb.ListReader<BOV>(BOV.reader).vTableGetNullable(_bc, _bcOffset, 4);

  @override
  String toString() {
    return 'BOVCOLLECTION{RECORDS: ${RECORDS}}';
  }
}

class _BOVCOLLECTIONReader extends fb.TableReader<BOVCOLLECTION> {
  const _BOVCOLLECTIONReader();

  @override
  BOVCOLLECTION createObject(fb.BufferContext bc, int offset) => 
    BOVCOLLECTION._(bc, offset);
}

class BOVCOLLECTIONBuilder {
  BOVCOLLECTIONBuilder(this.fbBuilder);
...
class BOVCOLLECTIONObjectBuilder extends fb.ObjectBuilder {
  final List<BOVObjectBuilder>? _RECORDS;

  BOVCOLLECTIONObjectBuilder({
    List<BOVObjectBuilder>? RECORDS,
  })
      : _RECORDS = RECORDS;
