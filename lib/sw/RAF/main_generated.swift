...
public enum rafPduType: Int8, Enum, Verifiable {
  ...
  public static var max: rafPduType { return .peerAbort }
  public static var min: rafPduType { return .bindInvocation }
}
...
public struct RAF: FlatBufferObject, Verifiable {
  ...
  public static var id: String { "$RAF" } 
  public static func finish(_ fbb: inout FlatBufferBuilder, end: Offset, prefix: Bool = false) { fbb.finish(offset: end, fileId: RAF.id, addPrefix: prefix) }
  ...
  public static func createRAF(
    ...
    PDU_TYPE: rafPduType = .bindInvocation,
    INITIATOR_IDOffset INITIATOR_ID: Offset = Offset(),
    RESPONDER_PORT_IDOffset RESPONDER_PORT_ID: Offset = Offset(),
    SERVICE_TYPE: UInt8 = 0,
    VERSION: UInt16 = 0,
    INVOKE_ID: UInt32 = 0,
    FRAME_QUALITY: UInt8 = 0,
    DATAVectorOffset DATA: Offset = Offset()
  ) -> Offset {
    let __start = RAF.startRAF(&fbb)
    RAF.add(PDU_TYPE: PDU_TYPE, &fbb)
    RAF.add(INITIATOR_ID: INITIATOR_ID, &fbb)
    RAF.add(RESPONDER_PORT_ID: RESPONDER_PORT_ID, &fbb)
    RAF.add(SERVICE_TYPE: SERVICE_TYPE, &fbb)
    RAF.add(VERSION: VERSION, &fbb)
    RAF.add(INVOKE_ID: INVOKE_ID, &fbb)
    RAF.add(FRAME_QUALITY: FRAME_QUALITY, &fbb)
    RAF.addVectorOf(DATA: DATA, &fbb)
    return RAF.endRAF(&fbb, start: __start)
  }
