...
impl<'a> STF<'a> {
  ...
  pub fn PRIMARY_CATEGORY(&self) -> capabilityClass {
    ...
    // Created from valid Table for this object
    // which contains a valid value in this slot
    unsafe { self._tab.get::<capabilityClass>(STF::VT_PRIMARY_CATEGORY, Some(capabilityClass::UNSPECIFIED)).unwrap()}
  }
...
impl<'a: 'b, 'b, A: ::flatbuffers::Allocator + 'a> STFBuilder<'a, 'b, A> {
  ...
  pub fn add_PRIMARY_CATEGORY(&mut self, PRIMARY_CATEGORY: capabilityClass) {
    self.fbb_.push_slot::<capabilityClass>(STF::VT_PRIMARY_CATEGORY, PRIMARY_CATEGORY, capabilityClass::UNSPECIFIED);
  }
  #[inline]
